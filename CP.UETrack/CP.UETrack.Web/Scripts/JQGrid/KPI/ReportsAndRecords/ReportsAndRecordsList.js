$(function () {
    var ReportMonthValues = "January:January; February:February; March:March; April:April; May:May; June:June; July:July; August:August; September:September;October:October; November:November; December:December";
    var statusVals = "Submitted:Submitted; Verified:Verified";
    genarateGrid(ReportMonthValues, statusVals);
});

function genarateGrid(ReportMonthVal, statusVals) {
    ...
    var ApprovalMonthVal = ReportMonthVal.replace(/[,;]$/, '').toString();
    var StatusVals = statusVals.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/reportsAndRecords/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Year', 'Month', 'Status'],
        colModel: [
                    { key: true, hidden: true, name: 'ReportsandRecordTxnId', width: '0%' },
                    { key: false, search: true, name: 'YearName', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    { key: false, name: 'MonthName', width: '30%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ApprovalMonthVal, defaultValue: "January" } },
                    { key: false, name: 'StatusName', width: '30%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusVals, defaultValue: "Submitted" } },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
         ...
         .append($("<input name='screenName' type='text'>").val("Reports_And_Records"))
         .append($("<input name='spName' type='text'>").val("uspFM_KPIReportsandRecordTxn_Export"))
