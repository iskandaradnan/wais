$(function () {
    ...
    genarateGrid(StockMonthValues);
});

function genarateGrid(StockMonthVal) {
    ...
    grid.jqGrid({
        url: "/api/MonthlyServiceFee/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Year', 'Month', 'Total Fee'],
        colModel: [
            { key: true, hidden: true, name: 'MonthlyFeeId', width: '0%' },
            { key: false, search: true, name: 'Year', width: '25%', searchoptions: { sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },            
            { key: false, name: 'Month', width: '25%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ApprovalMonthVal, defaultValue: "January" } },
            { key: false, search: true, name: 'TotalFee', width: '25%', searchoptions: { sopt: ['eq', 'ne'] } },            
            
            //{ name: 'edit', search: false, index: 'MonthlyFeeId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'MonthlyFeeDetId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'MonthlyFeeId', width: '5%', sortable: false, formatter: viewLink },
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='#' onclick=CheckMultipleTabs('/kpi/MonthlyServiceFee/Edit/" + rowObject.MonthlyFeeId + "') class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.MonthlyFeeId + ',' + '"' + rowdata.Month + '"' + ',' + '"'
     + rowdata.Year + '"' + ',' + '"' + gridID + '"'
...
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='#' onclick=CheckMultipleTabs('/kpi/MonthlyServiceFee/View/" + rowObject.MonthlyFeeId + "') class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("MonthlyServiceFee"))
        .append($("<input name='spName' type='text'>").val("uspFM_FinMonthlyFeeTxn_Export"))
