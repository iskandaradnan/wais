...
function genarateGrid() {
    ...
    var SearchBoxSelectFilterData = {
        Active: "Active:Active;Inactive:Inactive;".replace(/[,;]$/, '').toString()        
    }
    ...
    grid.jqGrid({
        ...
        colNames: ['FETC Id', 'Item Code', 'Item Description', 'Item Type', 'Status'],
        colModel: [
            { key: true, hidden: true, name: 'FETCId', width: '%' },
            { key: false, search: true, name: 'ItemCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ItemDescription', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ItemType', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'GridStatus', stype: "select", width: '20%', searchoptions: { sopt: ['eq'], value: SearchBoxSelectFilterData.Active } },
...
function activeText(cellValue, options, rowObject, action) {
    ...
    return rowObject.Status == 10392 ? "Active" : "Inactive";
}
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("FETC"))
        .append($("<input name='spName' type='text'>").val("sp_CLS_FETC_Export"))
