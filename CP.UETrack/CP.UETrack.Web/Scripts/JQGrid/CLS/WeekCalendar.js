...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/WeekCalendar/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['ID', 'Year'],
        colModel: [
            { key: false, hidden: true, search: false, name: 'ID', width: '20%' },
            { key: true, search: true, name: 'Year', width: '80%', searchoptions: { sopt: [ 'eq'] } },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("WeekCalendar"))
        .append($("<input name='spName' type='text'>").val("sp_CLS_WeekCalendar_Export"))
