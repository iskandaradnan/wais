...
function genarateGrid() {
    ...
    var SearchBoxPorteringStatus = {
        Active: "Not Assigned:Not Assigned;".replace(/[,;]$/, '').toString()
    }
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/ManualAssignPortering/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        //colNames: ['Id', 'Role', 'User Type', 'Status', 'Edit', 'Delete', 'View'],
        colNames: ['Id', 'Asset No.', 'Asset Tracker No.', 'Asset Tracker Date', 'Work Order No.', 'Work Flow Status'],
        colModel: [
            ...
            { key: false, search: true, name: 'PorteringNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            ...
            { key: false, search: true, name: 'WorkOrderNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
             { key: false, name: 'WorkFlowStatus', stype: "select", width: '15%', searchoptions: { sopt: ["eq", "ne"], value: SearchBoxSelectFilterData.Active } },
             //{ key: false, name: 'AssigneeType', stype: "select", width: '10%', searchoptions: { sopt: ["eq", "ne"], value: SearchBoxPorteringStatus.Active } },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Manual_Assign_Asset_Tracker"))
           .append($("<input name='headerColumnNames' type='text'>").val(headercolumns))
          .append($("<input name='spName' type='text'>").val("uspFM_GetNotAssignedPortering_GetAll_Export"))
