$(function () {
    var StatusValues = "Open:Open;Closed:Closed;";
    var carTypes = 'Auto Generation CAR:Auto Generation CAR;Manual CAR:Manual CAR';
    var carStatusValues = 'Approved:Approved;Rejected:Rejected;Submitted:Submitted';
    genarateGrid(StatusValues, carTypes, carStatusValues);
});

function genarateGrid(statusValues, carTypes, carStatusValues) {
    ...
    var CarStatusValues = carStatusValues.replace(/[,;]$/, '').toString();
    var CarTypes = carTypes.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/correctiveActionReport/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'CAR No.', 'CAR Date', 'Indicator', 'Assignee', 'Status', 'CAR Status', 'CAR Type'],
        colModel: [
                    { key: true,  hidden: true,  name: 'CarId', width: '0%' },
                    { key: false, search: true, name: 'CARNumber', width: '17%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    { key: false, search: true, name: 'CARDate', width: '17%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
                    //{ key: false, search: true, name: 'WorkOrderNumber', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    { key: false, search: true, name: 'IndicatorCode', width: '11%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    { key: false, search: true, name: 'Assignee', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    //{ key: false, search: true, name: 'FailureSymptomCode', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
                    { key: false, search: true, name: 'StatusValue', width: '10%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValues, defaultValue: "Open" } },
                    { key: false, search: true, name: 'CARStatusValue', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: CarStatusValues, defaultValue: "Approved" } },
                    { key: false, search: true, name: 'CARGeneration', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: CarTypes, defaultValue: "Auto Generation CAR" } },
        ],
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
{
    ...
    //    window.location.href = "/qap/correctiveactionreport/add";
    //}
    //    })
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
         ...
         .append($("<input name='screenName' type='text'>").val("Corrective_Action_Report"))
         .append($("<input name='spName' type='text'>").val("uspFM_QAPCarTxn_Export"))
