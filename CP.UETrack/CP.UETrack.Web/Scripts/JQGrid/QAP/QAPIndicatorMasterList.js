...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/QAPIndicatorMaster/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id','Indicator Number', 'Indicator Description'],
        colModel: [
            { key: true, hidden: true, name: 'QAPIndicatorId', width: '0%' },
            //{ key: false, search: true, name: 'Service', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'IndicatorNumber', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'IndicatorDescription', width: '40%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },            

            //{ name: 'edit', search: false, index: 'QAPIndicatorId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'QAPIndicatorId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'QAPIndicatorId', width: '5%', sortable: false, formatter: viewLink },
        ...
        //        CheckMultipleTabs("/QAP/QAPIndicatorMaster/add");
        //    }
        //}
        )
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='#' onclick=CheckMultipleTabs('/QAP/QAPIndicatorMaster/Edit/" + rowObject.QAPIndicatorId + "') class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.QAPIndicatorId + ',' + '"' + rowdata.QAPIndicatorId + '"' + ',' + '"'
     + rowdata.IndicatorCode + '"' + ',' + '"' + gridID + '"'
     + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";


}
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='#' onclick=CheckMultipleTabs('/QAP/QAPIndicatorMaster/View/" + rowObject.QAPIndicatorId + "') class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function confirmDelete(Id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/QAPIndicatorMaster/Delete/" + Id)

             .done(function (result) {
                 filterGrid();
                 showMessage('Indicator Master', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("QAP_Indicator_Master"))
          .append($("<input name='spName' type='text'>").val("uspFM_MstQAPIndicator_Export"))
