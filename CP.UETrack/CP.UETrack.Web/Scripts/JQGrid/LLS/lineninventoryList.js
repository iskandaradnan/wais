$(function () {
    var StoreTypeValue = "User Area/Dept:User Area/Dept;Central Clean Linen Store:Central Clean Linen Store;";
    
    genarateGrid(StoreTypeValue);
});

function genarateGrid(storeTypeValue) {
    ...
    var StoreTypeValue = storeTypeValue.replace(/[,;]$/, '').toString();
   
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/LinenInventory/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Year', 'Month', 'StoreType', 'DocumentNo', 'Date', 'TotalPcs'/*, 'TotalRecords'*/],
        colModel: [
            { key: true, hidden: true, name: 'LinenInventoryId', width: '0%' },
            { key: true, search: true, name: 'Year', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Month', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'StoreType', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StoreTypeValue, defaultValue: "User Area/Dept" } },
            { key: false, search: true, name: 'DocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
           // { key: false, search: true, name: 'Date', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Date', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'TotalPcs', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'TotalRecords', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (LinenInventoryId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LinenInventoryId);
            //console.log(rowData);
            LinkClicked(LinenInventoryId);
        },
        ...
        if (result) {
            $.get("/api/LinenInventory/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('LinenInventory Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LinenInventory_Details"))
        .append($("<input name='spName' type='text'>").val("LLSLinenInventoryTxn_Export"))
