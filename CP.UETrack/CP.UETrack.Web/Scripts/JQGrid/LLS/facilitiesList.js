$(function () {
    ...
    var ItemTypeValues = "Facilities: Facilities;Equipment: Equipment;Tools: Tools;Consumable: Consumable;";
   
    genarateGrid(StatusValues, ItemTypeValues );
});

function genarateGrid(statusValues, itemTypeValues ) {
    ...
    var ItemTypeValues = itemTypeValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/Facilities/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Item Code', 'Item Description', 'ItemType', 'Status', 'Effective FromDate ', 'EffectiveToDate'],
        colModel: [
            { key: true, hidden: true, name: 'FETCId', width: '0%' },
            { key: true, search: true, name: 'ItemCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ItemDescription', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ItemType', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ItemTypeValues, defaultValue: "Facilities" } },
            { key: false, search: true, name: 'Status', width: '10%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValues, defaultValue: "Active" } },
            { key: false, search: true, name: 'EffectiveFromDate', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'EffectiveToDate', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 16, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },   
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
        ...
        onCellSelect: function (FETCId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", FETCId);
            //console.log(rowData);
            LinkClicked(FETCId);
        },
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Facilities"))
        .append($("<input name='spName' type='text'>").val("LLSFacilityEquipToolsConsumeMst_Export"))
