...
$(function () {
    var StoreType = "Central Clean Linen Store:Central Clean Linen Store;Central Soiled Linen Store:Central Soiled Linen Store;";
    var Year = "2019:2019;2020:2020;";
    ...
    genarateGrid(StoreType, Year, Month);
});
function genarateGrid(storeType, year, month) {
    ...
    var StoreType = storeType.replace(/[,;]$/, '').toString();
    var Year = year.replace(/[,;]$/, '').toString();
    var Month = month.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/CentralLinenStoreHKeeping/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'StoreType ', 'Year', 'Month',],
        colModel: [
            { key: true, hidden: true, name: 'Id', width: '0%' },
            { key: true, serach: true, name: 'StoreType', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StoreType, defaultValue: "Central Clean Linen Store" } },
            { key: false, search: true, name: 'Year', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: Year, defaultValue: "2019" } },
            { key: false, search: true, name: 'Month', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: Month, defaultValue: "January" } },
        ...
        onCellSelect: function (HouseKeepingId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", HouseKeepingId);
            //console.log(rowData);
            LinkClicked(HouseKeepingId);
        },
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("CentralLinenStoreHKeeping_Details"))
        .append($("<input name='spName' type='text'>").val("LLSCentralLinenStoreHKeepingTxn_Export"))
