$(function () {
    var DespatchedFrom = "SC Tactical Sdn Bhd:SC Tactical Sdn Bhd;Test:Test;";
    //var UserTypeValues = "Company HQ User:Company HQ User;Company User:Company User;Facility User:Facility User;";
    genarateGrid(DespatchedFrom);
});

function genarateGrid(despatchedFrom) {
    ...
    var DespatchedFrom = despatchedFrom.replace(/[,;]$/, '').toString();
    //var UserTypeValues = userTypeValues.replace(/[,;]$/, '').toString();;
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/CleanLinenDespatch/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'DocumentNo ', 'Date Received', 'Despatched From', 'TotalWeightKg', 'TotalReceivedPcs '],
        colModel: [
            { key: true, hidden: true, name: 'CleanLinenDespatchId', width: '0%' },
            { key: true, serach: true, name: 'DocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'DateReceived', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge','cn'] } },
            { key: false, search: true, name: 'DateReceived', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y H:i" }, formatter: "date", resizable: true, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: true, format: 'd-M-Y H:i', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'Despatched From', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: DespatchedFrom, defaultValue: "SC Tactical Sdn Bhd" } },
            { key: false, search: true, name: 'TotalWeightKg', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalReceivedPcs', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'Remarks', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ],
        ...
        onCellSelect: function (CleanLinenDespatchId, iCol, content, event) {
            LinkClicked(CleanLinenDespatchId);
        ...
        if (result) {
            $.get("/api/CleanLinenDespatch/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('CleanLinenDespatch Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("CleanLinenDespatch_Details"))
        .append($("<input name='spName' type='text'>").val("LLSCleanLinenDespatchTxn_Export"))
