...
function genarateGrid(statusValues, ContractTypeValues, controllerName) {
    ...
    grid.jqGrid({
        url: "/api/LinenInjection/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'DocumentNo', 'InjectionDate', 'DONo', 'DODate', 'PONo '/*, 'TotalRecords'*/],
        colModel: [
            { key: true, hidden: true, name: 'LinenInjectionId', width: '0%' },
            { key: true, search: true, name: 'DocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'InjectionDate', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'InjectionDate', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'DONo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'DODate', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'DODate', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'PONo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (LinenInjectionId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LinenInjectionId);
            //console.log(rowData);
            LinkClicked(LinenInjectionId);
        },
        ...
        if (result) {
            $.get("/api/LinenInjection/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('LinenInjection Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LinenInjection_Details"))
        .append($("<input name='spName' type='text'>").val("LLSLinenInjectionTxn_Export"))
