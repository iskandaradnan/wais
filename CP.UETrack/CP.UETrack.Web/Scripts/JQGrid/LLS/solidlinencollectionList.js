$(function () {
    var CollectionScheduleValue = "Schedule 1:Schedule 1;Schedule 2:Schedule 2;Schedule 3:Schedule 3;";
   
    
    genarateGrid(CollectionScheduleValue);
});

function genarateGrid(collectionScheduleValue) {
    ...
    var CollectionScheduleValue = collectionScheduleValue.replace(/[,;]$/, '').toString();
   // var contractTypeValues = ContractTypeValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/SoiledLinenCollection/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'DocumentNo', 'CollectionDate', 'TotalWeight', 'LaundryPlantName', 'TotalWhiteBag', 'TotalRedBag', 'TotalGreenBag', 'TotalBrownBag'],
        colModel: [
            { key: true, hidden: true, name: 'SoiledLinenCollectionId', width: '0%' },
            { key: true, serach: true, name: 'DocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'CollectionDate', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'TotalWeight', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'LaundryPlantName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalWhiteBag', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalRedBag', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalGreenBag', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalBrownBag', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (SoiledLinenCollectionId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", SoiledLinenCollectionId);
            //console.log(rowData);
            LinkClicked(SoiledLinenCollectionId);
        },
        ...
        if (result) {
            $.get("/api/SoiledLinenCollection/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('SoiledLinenCollection Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("SoiledLinenCollection_Details"))
        .append($("<input name='spName' type='text'>").val("LLSSoiledLinenCollectionTxn_Export"))
