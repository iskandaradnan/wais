$(function () {
    ...
    genarateGrid(StatusValues, contractTypeValues, controllerName, Months);
});

function genarateGrid(statusValues, ContractTypeValues, controllerName, months) {
    ...
    var Months = months.replace(/[,;]$/, '').toString();
    var StatusValues = statusValues.replace(/[,;]$/, '').toString();
    var contractTypeValues = ContractTypeValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/LinenRejectReplacement/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Month', 'DateTime', 'DocumentNo', 'CLINo', 'UserAreaCode', 'UserLocationCode', 'TotalQuantityRejected','TotalQuantityReplaced'],
        colModel: [
            { key: true, hidden: true, name: 'LinenRejectReplacementId', width: '0%' },
            { key: true, search: true, name: 'Month', width: '10%', stype: "select", searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'], value: Months, defaultValue: "January"   } },
            { key: false, search: true, name: 'DateTime', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'DocumentNo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'CLINo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            ...
            { key: false, search: true, name: 'TotalQuantityRejected', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalQuantityReplaced', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
        ...
        onCellSelect: function (LinenRejectReplacementId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LinenRejectReplacementId);
            //console.log(rowData);
            LinkClicked(LinenRejectReplacementId);
        },
        ...
        if (result) {
            $.get("/api/LinenRejectReplacement/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('LinenRejectReplacement Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LinenRejectReplacement_Details"))
        .append($("<input name='spName' type='text'>").val("LLSLinenRejectReplacementTxn_Export"))
