$(function () {
    var LicenseTypeValue = "Driver:Driver;Vehicle:Vehicle;Laundry Plant:Laundry Plant;Weighing Scale:Weighing Scale;Equipment:Equipment;Manpower:Manpower;";
    ...
    genarateGrid(LicenseTypeValue, IssuingBodyValue);
});

function genarateGrid(licenseTypeValue, issuingBodyValue) {
    ...
    var LicenseTypeValue = licenseTypeValue.replace(/[,;]$/, '').toString();
    var IssuingBodyValue = issuingBodyValue.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/LicenseType/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'LicenseType', 'LicenseCode', 'LicenseDescription', 'IssuingBody',],
        colModel: [
            { key: true, hidden: true, name: 'LicenseTypeId', width: '0%' },
            { key: true, search: true, name: 'LicenseType', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: LicenseTypeValue, defaultValue: "Driver" } },
            { key: false, search: true, name: 'LicenseCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'LicenseDescription', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'IssuingBody', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: IssuingBodyValue, defaultValue: "Jabatan Pengangkutan Jalan" } },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
        ...
        onCellSelect: function (LicenseTypeId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LicenseTypeId);
            //console.log(rowData);
            LinkClicked(LicenseTypeId);
        },
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LicenseType"))
        .append($("<input name='spName' type='text'>").val("LLSLicenseTypeMstDet_Export"))
