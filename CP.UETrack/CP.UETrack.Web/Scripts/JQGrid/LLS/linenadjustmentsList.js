$(function () {
    var StatusValue = "open:open;closed:closed;";
...
function genarateGrid(statusValue) {
    ...
    grid.jqGrid({
        url: "/api/LinenAdjustments/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Year', 'Month', 'DocumentNo', 'DocumentDate', 'AuthorisedBy ', 'Status'],
        colModel: [
            { key: true, hidden: true, name: 'LinenAdjustmentId', width: '0%' },
            ...
            { key: false, search: true, name: 'DocumentDate', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AuthorisedBy', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
           // { key: false, search: true, name: 'DocumentNo', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Status', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValue, defaultValue: "Open" } },
            ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
        ...
        onCellSelect: function (LinenAdjustmentId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LinenAdjustmentId);
            //console.log(rowData);
            LinkClicked(LinenAdjustmentId);
        },
        ...
        if (result) {
            $.get("/api/LinenAdjustments/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('LinenAdjustments Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LinenAdjustments_Details"))
        .append($("<input name='spName' type='text'>").val("LLSLinenAdjustmentTxn_Export"))
