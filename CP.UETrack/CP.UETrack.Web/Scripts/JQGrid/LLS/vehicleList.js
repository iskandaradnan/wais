$(function () {
    ...
    var ManufacturerValue = "Hino: Hino;";
   
    genarateGrid(StatusValues, ManufacturerValue);
});

function genarateGrid(statusValues, manufacturerValue) {
    ...
    var ManufacturerValue = manufacturerValue.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/VechileDetails/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'VehicleNo', 'Manufacturer', 'LaundryPlantName', 'Status', 'EffectiveFrom', 'EffectiveTo', 'LoadWeight'],
        colModel: [
            { key: true, hidden: true, name: 'VehicleId', width: '0%' },
            { key: true, search: true, name: 'VehicleNo', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Manufacturer', width: '15%', searchoptions: { sopt: ['cn'] } },
            { key: false, search: true, name: 'LaundryPlantName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Status', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValues, defaultValue: "Active" } },
            { key: false, search: true, name: 'EffectiveFrom', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'EffectiveTo', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            //{ key: false, search: true, name: 'EffectiveFrom', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'EffectiveTo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'LoadWeight', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        width: '100%',
        viewrecords: true,
        //caption: 'License and Certificate Details',
        ...
        onCellSelect: function (VehicleId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", VehicleId);
            //console.log(rowData);
            LinkClicked(VehicleId);
        },
        ...
        if (result) {
            $.get("/api/VechileDetails/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('Vehicle Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Vehicle_Details"))
        .append($("<input name='spName' type='text'>").val("LLSVehicleDetailsMst_Export"))
