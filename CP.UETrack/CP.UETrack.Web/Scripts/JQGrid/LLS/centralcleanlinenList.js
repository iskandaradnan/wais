...
function genarateGrid(statusValues, ContractTypeValues, controllerName) {
    ...
    grid.jqGrid({
        url: "/api/CentralCleanLinen/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['StoreTypeID', 'StoreType', 'TotalRecords'],
        colModel: [
            { key: true, hidden: true, name: 'StoreTypeID', width: '0%' },
            { key: true, search: true, name: 'StoreType', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (StoreTypeID, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", StoreTypeID);
            //console.log(rowData);
            LinkClicked(StoreTypeID);
        },
...
function confirmDelete(id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/CentralCleanLinen/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('CentralCleanLinen', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("CentralCleanLinen"))
        .append($("<input name='spName' type='text'>").val("LLSCentralCleanLinenStoreMst_Export"))
