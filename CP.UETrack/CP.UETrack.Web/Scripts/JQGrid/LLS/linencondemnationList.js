...
function genarateGrid(statusValues, ContractTypeValues, controllerName) {
    ...
    grid.jqGrid({
        url: "/api/LinenCondemnation/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'DocumentNo', 'DocumentDate', 'InspectedBy', 'VerifiedBy', 'TotalCondemns',],
        colModel: [
            { key: true, hidden: true, name: 'LinenCondemnationId', width: '0%' },
            { key: true, serach: true, name: 'DocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'DocumentDate', width: '20%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'InspectedBy', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'VerifiedBy', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TotalCondemns', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (LinenCondemnationId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", LinenCondemnationId);
            //console.log(rowData);
            LinkClicked(LinenCondemnationId);
        },
        ...
        if (result) {
            $.get("/api/LinenCondemnation/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('LinenCondemnation Details', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("LinenCondemnation_Details"))
        .append($("<input name='spName' type='text'>").val("LLSLinenCondemnationTxn_Export"))
