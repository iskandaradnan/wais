$(function () {
    ...
    genarateGrid(StatusValue);
});

function genarateGrid(statusValue) {
    ...
    grid.jqGrid({
        url: "/api/DriverDetails/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'DriverCode', 'LaundryPlantName', 'DriverName', 'Status', 'EffectiveFrom', 'EffectiveTo', 'ContactNo'],
        colModel: [
            { key: true, hidden: true, name: 'DriverId', width: '0%' },
            { key: true, search: true, name: 'DriverCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'LaundryPlantName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'DriverName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            ...
            { key: false, search: true, name: 'ContactNo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
        ...
        onCellSelect: function (DriverId, iCol, content, event) {
            var rowData = $(this).jqGrid("getRowData", DriverId);
            //console.log(rowData);
            LinkClicked(DriverId);
        },
        ...
        if (result) {
            $.get("/api/DriverDetails/Delete/" + id)
                .done(function (result) {
                    filterGrid();
                    showMessage('DriverDetails', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Driver_Details"))
        .append($("<input name='spName' type='text'>").val("LLSDriverDetailsMst_Export"))
