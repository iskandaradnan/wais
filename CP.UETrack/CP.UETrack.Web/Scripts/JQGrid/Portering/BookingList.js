...
function genarateGrid() {
    ...
    grid.jqGrid({
        ...
        colNames: ['Id', 'Loaner / Test Equipment No.', 'Booking Start Date', 'Booking End Date', 'Work Order No.', 'Requestor', 'Booking Status', 'TypeCode', 'Type Description', 'Model', 'Manufacturer',],
        colModel: [
            { key: true, hidden: true, name: 'LoanerTestEquipmentBookingId', width: '0%' },
            { key: false, name: 'LoanerTestNo', width: '10', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'BookingStartFrom', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'BookingEnd', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'WorkOrderNo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'RequestorName', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'BookingStatusValue', stype: "select", width: '8%', searchoptions: { sopt: ["eq", "ne"], value: SearchBoxSelectFilterData.Active } },
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='sortColumnName' type='text'>").val(sortColumnName))
         .append($("<input name='screenName' type='text'>").val("Loaner_Test_Equipment_Booking"))
          .append($("<input name='spName' type='text'>").val("uspFM_EngLoanerTestEquipmentBookingTxn_Export "))
