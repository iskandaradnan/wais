...
* Modules: grid.base.js; jquery.fmatter.js; grid.custom.js; grid.common.js; grid.formedit.js; grid.filter.js; grid.inlinedit.js; grid.celledit.js; jqModal.js; jqDnR.js; grid.subgrid.js; grid.grouping.js; grid.treegrid.js; grid.import.js; JsonXml.js; grid.tbltogrid.js; grid.jqueryui.js; 
 Modified the pagining issue - hazarath  on 03 March 2016
 Modified the Reload grid issue with retaing filters  - hazarath - on 09 Jun 2016
 ...
 Modified the code trim search parameter values   - Prabhakaran S 07- JULY - 2017 6.50 pm
*/
...
f)
    ...
    altRows: !1, selarrrow: [], savedRow: [], shrinkToFit: !0, xmlReader: {}, jsonReader: {}, subGrid: !1, subGridModel: [], reccount: 0, lastpage: 0, lastsort: 0, selrow: null, beforeSelectRow: null, onSelectRow: null, onSortCol: null, ondblClickRow: null, onRightClickRow: null, onPaging: function (pgButton) {var x1, x2;if ("user" == pgButton) {x1 = b(".ui-pg-input").val();x2 = b(this).getGridParam("lastpage");if (x1 > x2) {b(".ui-pg-input").val(x2);b(this).setGridParam({ page: x2 }).trigger("reloadGrid");return 'stop';}if (x1 < 1) {b(".ui-pg-input").val(1);b(this).setGridParam({ page: 1 }).trigger("reloadGrid");return 'stop';}}}, onSelectAll: null, onInitGrid: null, loadComplete: null, gridComplete: null, loadError: null, loadBeforeSend: null, afterInsertRow: null, beforeRequest: null, beforeProcessing: null, onHeaderClick: null, viewrecords: !1, loadonce: !1, multiselect: !1, multikey: !1, editurl: null,
...
c.emptyRows.call(d[0],!1,!1)),c.populate(F))}}},scrollGrid:function(a){if(e.scroll){var b=c.bDiv.scrollTop;void 0===c.scrollTop&&(c.scrollTop=0);b!=c.scrollTop&&(c.scrollTop=b,c.timer&&clearTimeout(c.timer),c.timer=setTimeout(c.populateVisible,e.scrollTimeout))}c.hDiv.scrollLeft=c.bDiv.scrollLeft;e.footerrow&&(c.sDiv.scrollLeft=c.bDiv.scrollLeft);a&&a.stopPropagation()},selectionPreserver:function(a){var c=a.p,d=c.selrow,e=c.selarrrow?b.makeArray(c.selarrrow):null,f=a.grid.bDiv.scrollLeft,h=function(){var g;
    ...
    if (localStorage) {
        ...
        if (sd != null) {
            var id = parseInt(b("#screenIdforDocument").val());
            var objIndex = findIndex(sd, id);//sd.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex methode.
            if (objIndex > -1) {
                var dv = JSON.parse(localStorage.getItem('defaultData')); //Getting localstorage default filter value data values.
                var bTicket = false;
                var strRules = "rules";
                if (dv != null) {
                    var dvIndex = findIndex(dv, id);
                    if (dvIndex > -1) {
                        bTicket = dv[dvIndex].IsTicket;
                        if (bTicket == true) { strRules = "trules"; }
                    }
                }
                var pd = { "groupOp": "AND", "rules": sd[objIndex][strRules] };
                var ps = JSON.stringify(pd);
                //var pd ='{"groupOp":"AND","rules":[{"field":"Uploadeddate","op":"eq","data":"12-Jul-2017"}]}';//for reference 
                this.p.postData.filters = ps;
                this.p.postData.search = true;
                this.p.search = true;
            }
        }
    }
    this.p.useProp = !!b.fn.prop; var d, g; if (0 === this.p.colNames.length) for (d = 0; d < this.p.colModel.length; d++) this.p.colNames[d] = this.p.colModel[d].label || this.p.colModel[d].name; if (this.p.colNames.length !== this.p.colModel.length) alert(b.jgrid.errors.model); else {
...
}, u = b.jgrid.from(a.p.data); a.p.ignoreCase && (u = u.ignoreCase()); if (a.p.search === true) {
      var m = a.p.postData.filters; if (m) { typeof m === "string" && (m = b.jgrid.parse(m)); c(m) } else try { u = n[a.p.postData.searchOper](u)(a.p.postData.searchField, a.p.postData.searchString, f[a.p.postData.searchField]) } catch (p) { }
} if (a.p.grouping) for (o = 0; o < q; o++) u.orderBy(h[o], l.groupOrder[o], g[o].stype, g[o].srcfmt); d && a.p.sortorder && e && (a.p.sortorder.toUpperCase() ==
...
h = "", i, j, k, l, m = function (c) { var d; b.isFunction(a.p.onPaging) && (d = a.p.onPaging.call(a, c)); a.p.selrow = null; if (a.p.multiselect) { a.p.selarrrow = []; ca(false) } a.p.savedRow = []; return d == "stop" ? false : true }, c = c.substr(1), d = d + ("_" + c); i = "pg_" + c; j = c + "_left"; k = c + "_center"; l = c + "_right"; b("#" + b.jgrid.jqID(c)).append("<div id='" + i + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + j + "' align='center' style='background-color:#fff;'></td><td id='" + l + "' align='right' style='background-color:#fff; padding:10px;'></td><td id='" + k + "' align='right' style='white-space:pre;background-color:#fff;'></td></tr></tbody></table></div>").attr("dir", "ltr"); if (a.p.rowList.length > 0) { h = "<td dir='" + g + "'>"; h = h + "<select class='ui-pg-selbox' role='listbox'>"; for (j = 0; j < a.p.rowList.length; j++) h = h + ('<option role="option" value="' + a.p.rowList[j] + '"' + (a.p.rowNum == a.p.rowList[j] ? ' selected="selected"' : "") + ">" + a.p.rowList[j] + "</option>"); h = h + "</select></td>" } g == "rtl" && (f = f + h); a.p.pginput === true && (e = "<td dir='" +
g+"'>"+b.jgrid.format(a.p.pgtext||"","<input class='ui-pg-input' type='text' size='2' maxlength='5' value='0' role='textbox'/>","<span id='sp_1_"+b.jgrid.jqID(c)+"'></span>")+"</td>");if(a.p.pgbuttons===true){j=["first"+d,"prev"+d,"next"+d,"last"+d];g=="rtl"&&j.reverse();f=f+("<td id='"+j[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-first'></span></td>");f=f+("<td id='"+j[1]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-prev'></span></td>");
...
a.p.viewrecords===true&&b("td#"+c+"_"+a.p.recordpos,"#"+i).append("<div dir='"+g+"' style='text-align:center' class='ui-paging-info'></div>");b("td#"+c+"_"+a.p.pagerpos,"#"+i).append(f);h=b(".ui-jqgrid").css("font-size")||"11px";b(document.body).append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+h+";visibility:hidden;' ></div>");f=b(f).clone().appendTo("#testpg").width();b("#testpg").remove();if(f>0){e!==""&&(f=f+50);b("td#"+c+"_"+a.p.pagerpos,"#"+
...
} a.p.pginput === true && b("input.ui-pg-input", "#" + i).keypress(function (c)
{
    ...
    if (c.keyCode == 8 || c.keyCode == 13 || c.keyCode==46) {
        // let it happen, don't do anything
        if (c.keyCode == 13) {
            if (!m('user')) { return false; }
            a.p.page = b(this).val() > 0 ? b(this).val() : a.p.page; I();
          //  populate();
            return false;
        }
    }
...
}); n && a("#" + h + "_query").bind("click", function () { a(".queryresult", p).toggle(); return false }); void 0 === b.stringResult && (b.stringResult = b.multipleSearch);
a("#" + h + "_search").bind("click", function () {
     var d = a("#" + h), c = {}, g, k = d.jqFilter("filterData"); if (b.errorcheck) {
        d[0].hideError(); b.showQuery || d.jqFilter("toSQLString"); if (d[0].p.error) {
         ...
         if (typeof g === "string") {
             var jd = {
                 "MenuId": parseInt($("#screenIdforDocument").val()),
                 "rules": [], //page jqgrid filter rules
                 "trules": [] //Asis ticket jqgrid filter rules
             };
             var x = $.parseJSON(g);
             var sd = [];
             var dv = JSON.parse(localStorage.getItem('defaultData')); //Getting localstorage default filter value data values.
             ...
             a.each(x.rules, function (p, v) {
                  v.data=v.data.trim();
                 if (jd[strRules] == null) {
                     jd[strRules] = [v];//Set 1st object in screen specific rules.
                 }
                 else
                 {
                     jd[strRules].push(v); //Added screen specific rules in this list.
                 }
               })
             if (localStorage) {
                 var sd = JSON.parse(localStorage.getItem('sessData')); //Getting localstorage data values.
                 if (sd == null) { 
                     sd = [jd];//Set 1st object in storage local list
                 } else {
                     var objIndex = findIndex(sd, id);//sd.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex method.
                     if (objIndex == -1)
                     { sd.push(jd) }
                     else {
                         if (bTicket == true) { jd.rules = sd[objIndex].rules; }
                         sd[objIndex] = jd; ////Update object's.
                     }
                 }
             ...
             g = JSON.stringify(x)
             c[b.sFilter] = g;
             a.each([b.sField, b.sValue, b.sOper], function () { c[this] = "" })
         }
        
     } else
         if (b.multipleSearch) { c[b.sFilter] = k; a.each([b.sField, b.sValue, b.sOper], function () { c[this] = "" })   }
         else { c[b.sField] = k.rules[0].field; c[b.sValue] = k.rules[0].data; c[b.sOper] = k.rules[0].op; c[b.sFilter] = "" } e.p.search = true;
    ...
    if (localStorage) {
        ...
        if (sd != null) {
            ...
            if (objIndex > -1)
            {
                ...
                var dv = JSON.parse(localStorage.getItem('defaultData')); //Getting localstorage default filter value data values.
                if (dv != null) {
                    var dvIndex = findIndex(dv, id);// dv.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex method.
                    if (dvIndex > -1) {
                        bTicket = dv[dvIndex].IsTicket;
                    }
                }
                if (bTicket == false) {
                    ...
                    sd[objIndex].trules = dv[dvIndex].trules;
                    }
                localStorage.setItem('sessData', JSON.stringify(sd));
            }
        }
    }
        var d = {}, c = a("#" + h); e.p.search = false; b.multipleSearch === false ? d[b.sField] = d[b.sValue] = d[b.sOper] = "" : d[b.sFilter] = "";
        c[0].resetFilter(); l && a(".ui-template", p).val("default");
        a.extend(e.p.postData, d); a(e).triggerHandler("jqGridFilterReset");
...
{gbox:"#gbox_"+a.jgrid.jqID(f.p.id),jqModal:!0,drag:!0,resize:!0,caption:c.alertcap,top:c.alerttop,left:c.alertleft,width:c.alertwidth,height:c.alertheight,closeOnEscape:c.closeOnEscape,zIndex:c.alertzIndex},"#gview_"+a.jgrid.jqID(f.p.id),a("#gbox_"+a.jgrid.jqID(f.p.id))[0],!0));var m=1,w,d=function(){a(this).hasClass("ui-state-disabled") },n=function(){a(this).removeClass("ui-state-hover")};c.cloneToTop&&f.p.toppager&&(m=2);for(w=0;w<m;w++){var g=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),
...
"</div>"), a("tr", g).append(i), a(i, g).attr({ title: c.refreshtitle || "", id: "refresh_" + l }).click(function () {
    if (!a(this).hasClass("ui-state-disabled")) {
        ...
        //f.p.search = false;
            try {
                var b = f.p.id;
                //f.p.postData.filters = ""; a("#fbox_" + a.jgrid.jqID(b)).jqFilter("resetFilter"); a.isFunction(f.clearToolbar) && f.clearToolbar.call(f, false)
...
(function (a) {
    a.fn.jqFilter = function (d) {
        if ("string" === typeof d) {
            ...
            if (!n) throw "jqFilter - No such method: " + d; var u = a.makeArray(arguments).slice(1);
        ...
        if (localStorage) {
            ...
            var sd = JSON.parse(localStorage.getItem('sessData')); //Getting localstorage data values.
            var dv = JSON.parse(localStorage.getItem('defaultData')); //Getting localstorage default filter value data values.
            var de = { //Get default filter values from grid first time loading and set this value in localstorage defaultdata.
                "MenuId": parseInt($("#screenIdforDocument").val()),
                "rules": d.filter.rules,
                "IsTicket": false,
                "trules": [{ "field": "TicketNo", "data": "", "op": "cn" }]
            };
            var objIndex = -1;
            if (dv == null) {
                dv = [de];//Set 1st object in storage local list
            } else {
                objIndex = findIndex(dv, id);// dv.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex method.
                if (objIndex == -1)
                { dv.push(de) }
            }
            if (sd != null) {
                var sdIndex = findIndex(sd, id);// sd.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex methode.
                if (sdIndex > -1) {
                    if (objIndex == -1) { //set session page filter
                        d.filter.rules = sd[sdIndex].rules;
                    }
                    else { // set asis ticket session filter
                        if (dv[objIndex].IsTicket == true) {
                            d.filter.rules = sd[sdIndex].trules;
                        }
                    }
                }
            }
...
description: "does not contain", operator: "NOT LIKE"}, { name: "nu", description: "is null", operator: "IS NULL" }, { name: "nn", description: "is not null", operator: "IS NOT NULL" }], numopts: "eq,ne,lt,le,gt,ge,nu,nn".split(","), stropts: "eq,ne,bw,bn,ew,en,cn,nc,nu,nn".split(","), strarr: ["text", "string", "blob"], _gridsopt: [], groupOps: [{ op: "AND", text: "AND" }, { op: "OR", text: "OR" }], groupButton: !0, ruleButtons: !0, direction: "ltr"
        }, a.jgrid.filter, d || {});return this.each(function () {if (!this.filter) {this.p = p;if (null ===this.p.filter || void 0 === this.p.filter) this.p.filter = { groupOp: this.p.groupOps[0].op, rules: [], groups: [] }; var d, n = this.p.columns.length, f, t = /msie/i.test(navigator.userAgent) && !window.opera; if (this.p._gridsopt.length) for (d = 0; d < this.p._gridsopt.length; d++) this.p.ops[d].description = this.p._gridsopt[d];
this.p.initFilter = a.extend(!0, {}, this.p.filter);if (n) {for (d = 0; d < n; d++) if (f = this.p.columns[d], f.stype ? f.inputtype = f.stype : f.inputtype || (f.inputtype = "text"), f.sorttype ? f.searchtype = f.sorttype : f.searchtype ||
...
}; this.onchange = function () { this.p.error = !1; this.p.errmsg = ""; return a.isFunction(this.p.onChange) ? this.p.onChange.call(this, this.p) : !1 };
this.reDraw = function (str) {
    ...
    if (objIndex > -1) {
        if (dv[objIndex].IsTicket == false) {
            if (sd != null && str == undefined) {
                var sdIndex = findIndex(sd, id);// sd.findIndex((obj => obj.MenuId == id));//Find index of specific object using findIndex method.
                if (sdIndex == -1) {
                    if (objIndex > -1) {
                        p.filter.rules = dv[objIndex].rules;
            ...
            if (str == undefined) {
                p.filter.rules = dv[objIndex].trules;
            }
        }
      }
    }
        a("table.group:first", this).remove(); 
    var g=this.createTableForGroup(p.filter,null);a(this).append(g);a.isFunction(this.p.afterRedraw)&&this.p.afterRedraw.call(this,this.p)};
this.createTableForGroup = function (g, k) {
    var b = this, c, e = a("<table class='group ui-widget ui-widget-content' style='border:0px none;'><tbody></tbody></table>"), d = "left"; "rtl" == this.p.direction && (d = "right", e.attr("dir", "rtl")); null === k && e.append("<tr class='error' style='display:none;'><th colspan='5' class='ui-state-error' align='" + d + "'></th></tr>"); var h = a("<tr></tr>"); e.append(h); d = a("<th colspan='5' align='" + d + "'></th>"); h.append(d); if (!0 === this.p.ruleButtons) {
        var i = a("<select class='opsel' style='width:250px;'></select>");
...
a.inArray(l.searchtype,b.p.strarr)!==-1?b.p.stropts:b.p.numopts;g.rules.push({field:l.name,op:e[0],data:""});b.reDraw('add');return false});d.append(h)}null!==k&&(h=a("<input type='button' value='-' title='Delete group' class='delete-group'/>"),d.append(h),h.bind("click",function(){for(c=0;c<k.groups.length;c++)if(k.groups[c]===g){k.groups.splice(c,1);break}b.reDraw();b.onchange();return false}));if(void 0!==g.groups)for(c=0;c<g.groups.length;c++)d=a("<tr></tr>"),e.append(d),h=a("<td class='first'></td>"),
...
this.createTableRowForRule = function (g, d) {
        var b = this, c = a("<tr></tr>"), e, f, h, i, j = "", l; c.append("<td class='first'></td>"); var m = a("<td class='columns'></td>"); c.append(m); var n = a("<select style='width:250px;'></select>"), o, q = []; m.append(n); n.bind("change", function () {
            g.field =
...
!0);if("nu"==g.op||"nn"==g.op)a(m).attr("readonly","true"),a(m).attr("disabled","true");var s=a("<select class='selectopts' style='width:250px;'></select>");j.append(s);s.bind("change",function(){g.op=a(s).val();h=a(this).parents("tr:first");var c=a(".input-elm",h)[0];if(g.op==="nu"||g.op==="nn"){g.data="";c.value="";c.setAttribute("readonly","true");c.setAttribute("disabled","true")}else{c.removeAttribute("readonly");c.removeAttribute("disabled")}b.onchange()});f=i.searchoptions.sopt?i.searchoptions.sopt:b.p.sopt?b.p.sopt:
-1!==a.inArray(i.searchtype,b.p.strarr)?b.p.stropts:b.p.numopts;j="";a.each(b.p.ops,function(){q.push(this.name)});for(e=0;e<f.length;e++)o=a.inArray(f[e],q),-1!==o&&(l=g.op===b.p.ops[o].name?" selected='selected'":"",j+="<option value='"+b.p.ops[o].name+"'"+l+">"+b.p.ops[o].description+"</option>");s.append(j);j=a("<td class='data'></td>");c.append(j);j.append(m);a.jgrid.bindEv(m,i.searchoptions,b);a(m).addClass("input-elm").css('width', '250px').bind("change",function(){g.data=i.inputtype==="custom"?i.searchoptions.custom_value(a(this).children(".customelement:first"),
"get"):a(this).val();b.onchange()});j=a("<td></td>");c.append(j);!0===this.p.ruleButtons&&(m=a("<input type='button' value='-' title='Delete rule' class='delete-rule ui-del'/>"),j.append(m),m.bind("click",function(){for(e=0;e<d.rules.length;e++)if(d.rules[e]===g){d.rules.splice(e,1);break}b.reDraw('delete');b.onchange();return false}));return c};this.getStringForGroup=function(a){var d="(",b;if(void 0!==a.groups)for(b=0;b<a.groups.length;b++){1<d.length&&(d+=" "+a.groupOp+" ");try{d+=this.getStringForGroup(a.groups[b])}catch(c){alert(c)}}if(void 0!==
...
d.p.columns[h];break}i&&r(f.data,i)}c+=f.op+"(item."+f.field+",'"+f.data+"')"}c+=")";return"()"===c?"":c}var d=this;return a(this.p.filter)};this.reDraw();if(this.p.showQuery)this.onchange();this.filter=!0}}})};
