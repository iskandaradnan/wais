 ...
 * Date: 2013-01-30
 * Modified the onPaging text box  maxlength = 5   - Prabhakaran S 03- DEC - 2016 12.55 Am
 * Modified the onPaging text box  allow only numeric value   - Prabhakaran S 03- DEC - 2016 12.55 Am
 */
//jsHint options
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var p = $.extend(true, {
                ...
                onPaging: function (pgButton) { var x1, x2; if ("user" == pgButton) { x1 = $(".ui-pg-input").val(); x2 = $(this).getGridParam("lastpage"); if (x1 > x2) { $(".ui-pg-input").val(x2); $(this).setGridParam({ page: x2 }).trigger("reloadGrid"); return 'stop'; } if (x1 < 1) { $(".ui-pg-input").val(1); $(this).setGridParam({ page: 1 }).trigger("reloadGrid"); return 'stop'; } } },
                onSelectAll: null,
                onInitGrid: null,
                loadComplete: null,
                gridComplete: null,
            ...
            setPager = function (pgid, tp) {
                ...
                if (dir == "rtl") { pgl += str; }
                if (ts.p.pginput === true) { pginp = "<td dir='" + dir + "'>" + $.jgrid.format(ts.p.pgtext || "", "<input class='ui-pg-input' type='text' size='2' maxlength='5' value='0' role='textbox'/>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + "</td>"; }
                if (ts.p.pgbuttons === true) {
                ...
                ruleFieldSelect.bind('change', function () {
                    ...
                    for (i = 0 ; i < op.length; i++) {
                        ...
                        if (ina !== -1) {
                            ...
                            if (that.p.ops[ina].name == "cn") { selected = " selected='selected'"; }
                            s += "<option value='" + that.p.ops[ina].name + "'" + selected + ">" + that.p.ops[ina].description + "</option>";
                            so++;
                        }
                    }
                    $(".selectopts", trpar).empty().append(s);
                    //$(".selectopts", trpar)[0].selectedIndex = 0;
                    $(".selectopts", trpar).selectedValue = "cn";
