...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/MasterLevel/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Facility', 'Level Name', 'Block Name', 'Short Name', 'Level Code', 'Status'],
        colModel: [
            { key: true, hidden: true, name: 'LevelId', width: '0%' },
            { key: false, name: 'FacilityName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'LevelName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: false, name: 'BlockName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'ShortName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'LevelCode', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'Active', stype: "select", width: '20%', formatter: activeText, searchoptions: { sopt: ["eq", "ne"], value: SearchBoxSelectFilterData.Active, defaultValue: "1" } },
            //{ name: 'edit', search: false, index: 'LevelId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'LevelId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'LevelId', width: '5%', sortable: false, formatter: viewLink },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("MasterLevel"))
        .append($("<input name='spName' type='text'>").val("uspFM_MstLocationLevel_Export"))
