...
function genarateGrid(serviceValues, userTypeValues) {
    ...
    grid.jqGrid({
        url: "/api/facilitystaffmst/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', /*'Facility Name',*/ 'Staff/Employee Id', 'Staff Name', /* 'Nationality',*/ 'Gender', 'Email', 'Status', 'Edit', 'Delete', 'View'],
        colModel: [
            { key: true, hidden: true, name: 'StaffMasterId', width: '0%' },
            //{ key: false, search: true, name: 'FacilityName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'StaffEmployeeId', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc'] } },
            { key: false, name: 'StaffName', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc'] } },
            //{ key: false, name: 'NationalityName', width: '15%', searchoptions: { sopt: ['cn', "eq", "ne"] } },
            { key: false, name: 'Gender', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: SearchBoxSelectFilterData.Gender, defaultValue: "Male" } },
            { key: false, name: 'Email', width: '20%', searchoptions: { sopt: ['cn', "eq", "ne"] } },
            { key: false, name: 'Status', width: '10%', stype: "select", formatter: activeText, searchoptions: { sopt: ["eq", "ne"], value: SearchBoxSelectFilterData.Active, defaultValue: "1" } },
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
        {
            ...
            onClickButton: function () {
                window.location.href = "/gm/facilitystaffmst/add";
            }
        }
        )
        .navButtonAdd('#grid_toppager', {
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='/gm/facilitystaffmst/edit/" + rowObject.StaffMasterId + "' class='ui-icon ui-icon-pencil'></a>";
}
...
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/gm/facilitystaffmst/view/" + rowObject.StaffMasterId + "' class='glyphicon glyphicon-eye-open' ></a>";
}
...
function confirmDelete(id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/facilitystaffmst/Delete/" + id)
             .done(function (result) {
                 filterGrid();
                 showMessage('Facility Staff', CURD_MESSAGE_STATUS.DS);
                 $('#myPleaseWait').modal('hide');
             })
             .fail(function () {
                 showMessage('Facility Staff', CURD_MESSAGE_STATUS.DF);
                 $('#myPleaseWait').modal('hide');
             });
        }
    });
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_MstStaff_Facility_Export"))
