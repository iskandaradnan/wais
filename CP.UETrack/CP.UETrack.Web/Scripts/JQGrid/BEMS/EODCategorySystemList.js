...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/eodcategorysystem/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Service', 'Category / System Name', 'Edit', 'Delete', 'View'],
        colModel: [
            { key: true, hidden: true, name: 'CategorySystemId', width: '0%' },
            { key: false, search: true, name: 'ServiceName', width: '40%', searchoptions: { sopt: ['cn', 'eq', 'ne'] } },
            { key: false, search: true, name: 'CategorySystemName', width: '45%', searchoptions: { sopt: ['cn', 'eq', 'ne'] } },
            { name: 'edit', search: false, index: 'CategorySystemId', width: '5%', sortable: false, formatter: editLink },
            { name: 'delete', search: false, index: 'CategorySystemId', width: '5%', sortable: false, formatter: deleteLink },
            { name: 'view', search: false, index: 'CategorySystemId', width: '5%', sortable: false, formatter: viewLink }
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
        {
            ...
            onClickButton: function () {
                window.location.href = "/bems/eodcategorysystem/add";
            }
        }
        )
        .navButtonAdd('#grid_toppager', {
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='/bems/eodcategorysystem/edit/" + rowObject.CategorySystemId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
//       return "<a href='javascript:confirmDelete(" + rowdata.CategorySystemId + ',' + '"' + Name + '"' + ',' + '"'
//        + ServiceName + '"' + ',' + '"' + gridID + '"'
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.CategorySystemId + ',' + '"' + rowdata.Service + '"' + ',' + '"'
     + rowdata.CategorySystemName + '"' + ',' + '"' + gridID + '"'
     + ")'class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
}

function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/bems/eodcategorysystem/view/" + rowObject.CategorySystemId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;' ></a>";
}
...
function confirmDelete(id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/eodcategorysystem/Delete/" + id)
             .done(function (result) {
                 filterGrid();
                 showMessage('Category System ', CURD_MESSAGE_STATUS.DS);
             ...
             .fail(function (response) {
                 ...
                 if (response.status == 400) {
                     ...
                     showMessage('Category', CURD_MESSAGE_STATUS.RAU);
                 }
                 $('#myPleaseWait').modal('hide');
             });
        }
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("EOD_Category/System_Master"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_EngEODCategorySystem_Export"))
