...
function genarateGrid() {
    ...
    var SearchBoxSelectFilterDataService = {
        Service: "BEMS:BEMS;".replace(/[,;]$/, '').toString()
    }

    var SearchBoxSelectFilterDataFacTyp = {
        FacilityType: "Resource Center:Resource Center;Storage:Storage;Workshop:Workshop;".replace(/[,;]$/, '').toString()
    }
    var SearchBoxSelectFilterDataCat = {
        Category: "Equipment Under Repair:Equipment Under Repair; PPE:PPE; Spare Parts and Consumables:Spare Parts and Consumables; Loaner:Loaner; Machinery:Machinery; Test Equipment:Test Equipment; Tools:Tools;".replace(/[,;]$/, '').toString()
    }

    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/FacilityWorkshop/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['FacilitiesWorkshopId', 'Year',  'Type of Facility', 'Category'],
        colModel: [
            { key: true, hidden: true, name: 'FacilitiesWorkshopId', width: '0%' },
            { key: false, search: true, name: 'Year', width: '30%', searchoptions: { sopt: [ 'eq', 'ne'] } },
           // { key: false, search: true, name: 'Service', stype: "select", width: '25%', formatter: activeText, searchoptions: { sopt: ['eq', 'ne'], value: SearchBoxSelectFilterDataService.Service, defaultValue: "BEMS" } },
           // { key: false, search: true, name: 'Service', stype: "select", width: '25%', formatter: activeText, searchoptions: { sopt: ['eq', 'ne'], value: SearchBoxSelectFilterDataService.Service, defaultValue: "BEMS" } },
            //{ key: false, search: true, name: 'Service', width: '20%', searchoptions: { sopt: ['eq', 'ne']} },
            { key: false, search: true, name: 'FacilityType', stype: "select", width: '30%', formatter: activeFacTyp, searchoptions: { sopt: ['eq', 'ne'], value: SearchBoxSelectFilterDataFacTyp.FacilityType, defaultValue: "Resource Center" } },
            //{ key: false, search: true, name: 'FacilityType', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc'] } },
            { key: false, search: true, name: 'Category', stype: "select", width: '40%', formatter: activeCat, searchoptions: { sopt: ['eq', 'ne'], value: SearchBoxSelectFilterDataCat.Category, defaultValue: "Equipment Under Repair" } },
            //{ key: false, search: true, name: 'Category', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc'] } },


            //{ name: 'edit', search: false, index: 'FacilitiesWorkshopId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'FacilitiesWorkshopId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'FacilitiesWorkshopId', width: '5%', sortable: false, formatter: viewLink },
...
function activeText(cellValue, options, rowObject, action) {
    ...
    return rowObject.ServiceId == 2 ? "BEMS" : "";
}


function activeFacTyp(cellValue, options, rowObject, action) {
    if (rowObject.FacilityTypeLovId == 101) {
        var factyp = "Resource Center";
    }
    else if (rowObject.FacilityTypeLovId == 102) {
        var factyp = "Storage";
    }
    else if (rowObject.FacilityTypeLovId == 103) {
        var factyp = "Workshop";
    }
    return factyp;
}

function activeCat(cellValue, options, rowObject, action) {
    if (rowObject.CategoryLovId == 104) {
        var cat = "Equipment Under Repair";
    }
    else if (rowObject.CategoryLovId == 105) {
        var cat = "PPE";
    }
    else if (rowObject.CategoryLovId == 106) {
        var cat = "Spare Parts and Consumables";
    }
    if (rowObject.CategoryLovId == 107) {
        var cat = "Loaner";
    }
    else if (rowObject.CategoryLovId == 108) {
        var cat = "Machinery";
    }
    else if (rowObject.CategoryLovId == 109) {
        var cat = "Test Equipment";
    }
    else if (rowObject.CategoryLovId == 110) {
        var cat = "Tools";
    }
    else if (rowObject.FacilityTypeLovId == 101) {
        var cat = "";
    }
    return cat;
}
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Facilities_&_Workshop"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_EngFacilitiesWorkshopTxn_Export"))
