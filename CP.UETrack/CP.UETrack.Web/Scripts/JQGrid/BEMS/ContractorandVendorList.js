$(function () {
    ...
    $.get("/api/ContractorandVendor/LoadCountry")
       .done(function (result) {
         ...
         //  var loadResult = JSON.parse(result);
          ...
          // $("#jQGridCollapse1").click();
           ...
           var res = result.CountryList;
           if (res.length > 0) {
               for (i = 0; i < res.length; i++) {
                   if (i >= 0) {
                       monthValues += res[i].LovId + ':' + res[i].FieldValue + ';';
                   }
               }
           }
           monthValues = monthValues.replace(/[,;]$/, '');

           monthValues = monthValues.toString();
           genarateGrid(ServiceValues, monthValues);
          // alert(monthValues);
...
function genarateGrid(serviceValues, monthValues) {
    ...
    //alert(monthValues);
    //var CountryValues = monthValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/ContractorandVendor/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Contractor / Vendor Registration Number', 'Contractor / Vendor Name','State','Country', 'Status'],
        colModel: [
            { key: true, hidden: true, name: 'ContractorId', width: '0%' },
            { key: false, search: true, name: 'SSMRegistrationCode', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ContractorName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
             { key: false, search: true, name: 'State', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
              //{ key: false, search: true, name: 'Country', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
             { key: false, name: 'Country', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: monthValues } },
            { key: false, name: 'StatusValue', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ServiceValues, defaultValue: "Active" } },
            //{ name: 'edit', search: false, index: 'ContractorId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'ContractorId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'ContractorId', width: '5%', sortable: false, formatter: viewLink },
        ],
        ...
        width: '100%',
        viewrecords: true,
        //caption: 'Contractor and Vendor Master',
        ...
        //        CheckMultipleTabs("/BEMS/ContractorandVendor/Add");
        //    }
        //}
        //)
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='sortColumnName' type='text'>").val(sortColumnName))
         .append($("<input name='screenName' type='text'>").val("Contractor_&_Vendor"))
        ...
        .append($("<input name='spName' type='text'>").val("uspFM_MstContractorandVendor_Export"))
