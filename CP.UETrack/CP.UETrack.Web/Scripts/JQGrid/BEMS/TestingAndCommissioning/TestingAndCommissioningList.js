$(function()
{
    var StatusValues = "Approved: Approved; Cancelled: Cancelled; Keep in view: Keep in view; Rejected: Rejected; Submitted: Submitted; Verified: Verified;";
    var TandCStatusValues = "Passed: Passed; Failed: Failed; Keep in view: Keep in view";
    genarateGrid(StatusValues, TandCStatusValues);
});

function genarateGrid(statusValues, tandCStatusValues)
{
    ...
    var hasAddPermission = Enumerable.From(JSON.parse($('#ActionPermissionValues').val())).Any("$.ActionPermissionName=='Add'");
        ...
        var TandCStatusValues = tandCStatusValues.replace(/[,;]$/, '').toString();

        var grid = $("#grid"),
        prmSearch = { multipleSearch: true, overlay: false };
            grid.jqGrid({
                url: "/api/testingAndCommissioning/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'T&C Document No.', 'T&C Date', 'T&C Status', 'Request No', 'Asset No.', 'Asset Category', 'Status'],  //, 'Edit', 'Verify', 'Approve', 'Reject', 'Delete', 'View'
        colModel: [
            { key: true, hidden: true, name: 'TestingandCommissioningId', width: '0%' },
            { key: false, search: true, name: 'TandCDocumentNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TandCDate', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'TandCStatusName', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: TandCStatusValues, defaultValue: "Passed" } },
            { key: false, search: true, name: 'RequestNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetCategory', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'StatusName', width: '15%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValues, defaultValue: "Approved" } },
        ],
...
function editLink(cellValue, options, rowObject, action) {
    if (rowObject.StatusId == 288)
                return "<a href='/bems/testingandcommissioning/edit/" + rowObject.TestingandCommissioningId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
            else
        ...
        function verifyLink(cellValue, options, rowObject, action) {
            if (rowObject.StatusId == 286)
                return "<a href='/bems/testingandcommissioning/verify/" + rowObject.TestingandCommissioningId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
            else
                return "<a href='javascript:NotificationAlert()' class='glyphicon glyphicon-ban-circle' style='cursor:default;color:#477ca9;font-size: 13px;'></a>";
        }
        function approveLink(cellValue, options, rowObject, action) {
            if (rowObject.StatusId == 289)
                return "<a href='/bems/testingandcommissioning/approve/" + rowObject.TestingandCommissioningId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
        else
                return "<a href='javascript:NotificationAlert()' class='glyphicon glyphicon-ban-circle' style='cursor:default;color:#477ca9;font-size: 13px;'></a>";
        }
        function rejectLink(cellValue, options, rowObject, action) {
            if (rowObject.StatusId == 289)
                return "<a href='/bems/testingandcommissioning/reject/" + rowObject.TestingandCommissioningId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
        else
                return "<a href='javascript:NotificationAlert()' class='glyphicon glyphicon-ban-circle' style='cursor:default;color:#477ca9;font-size: 13px;'></a>";
        }
        function NotificationAlert() {
            ...
            var documentNo = escapeShowTitle(rowdata.TandCDocumentNo);
            var tandcDate = rowdata.TandCDate.split(' ')[0];

            if (tandcDate != null && tandcDate != '')
            {
                var dateParts = tandcDate.split('/');
                var newDate = dateParts[1] + '/' + dateParts[0] + '/' + dateParts[2];
                tandcDate = moment(newDate).format("DD-MMM-YYYY")
            }

            return "<a href='javascript:confirmDelete(" +rowdata.TestingandCommissioningId + ',' + '"' + documentNo + '"' + ',' + '"'
             + tandcDate + '"' + ',' + '"' + gridID + '"'
             + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
        }
            function viewLink(cellValue, options, rowObject, action) {
                return "<a href='/bems/testingandcommissioning/view/" + rowObject.TestingandCommissioningId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
        }
            ...
            var $downloadForm = $("<form method='POST'>")
                ...
                .append($("<input name='screenName' type='text'>").val("Testing_&_Commissioning"))
                  .append($("<input name='spName' type='text'>").val("uspFM_EngTestingandCommissioningTxn_Export"))
