...
function genarateGrid() {
    ...
    grid.jqGrid({
        ...
        colNames: ['Id', 'Facility Code', 'Facility Name', 'Customer Name', 'State', 'Country', 'Active From', 'Active To'],
        colModel: [
            { key: true, hidden: true, name: 'FacilityId', width: '0%' },
            ...
            { key: false, search: true, name: 'State', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Country', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ActiveFrom', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, search: true, name: 'ActiveTo', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },           
        ],
        ...
        //        CheckMultipleTabs("/BEMS/Facility/Add");
        //    }
        //}
        //)
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='#' onclick=CheckMultipleTabs('/BEMS/Facility/Edit/" + rowObject.FacilityId + "') class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.FacilityId + ',' + '"' + rowdata.FacilityName + '"' + ',' + '"'
     + rowdata.FacilityCode + '"' + ',' + '"' + gridID + '"'
     + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";


}
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='#' onclick=CheckMultipleTabs('/BEMS/Facility/View/" + rowObject.FacilityId + "') class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
         ...
         .append($("<input name='screenName' type='text'>").val("Facility"))
        //.append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          ...
          .append($("<input name='spName' type='text'>").val("uspFM_MstLocationFacility_Export"))
