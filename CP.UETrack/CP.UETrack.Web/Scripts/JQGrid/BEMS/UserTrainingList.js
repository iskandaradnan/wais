...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/UserTraining/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['TrainingScheduleId', 'Training Schedule No.', 'Training Module', 'Training Description', 'Year', 'Quarter', 'Planned Date', 'Actual Date','Status'],
        colModel: [
            { key: true, hidden: true, name: 'TrainingScheduleId', width: '0%' },
            { key: false, search: true, name: 'TrainingScheduleNo', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Trainingmodule', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TrainingDescription', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Year', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'QuarterVal', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'PlannedDate', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            //{ key: false, search: true, name: 'PlannedDate', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'ActualDate', width: '10%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            //{ key: false, search: true, name: 'ActualDate', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Status', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ name: 'edit', search: false, index: 'TrainingScheduleId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'TrainingScheduleId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'TrainingScheduleId', width: '5%', sortable: false, formatter: viewLink }
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='/bems/UserTraining/edit/" + rowObject.TrainingScheduleId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.TrainingScheduleId + ',' + '"' + rowdata.TrainingScheduleNo + '"' + ',' + '"'
     + rowdata.TrainingDescription + '"' + ',' + '"' + gridID + '"'
     + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
}

function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/bems/UserTraining/view/" + rowObject.TrainingScheduleId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;' ></a>";
}
        ...
        if (result) {
            $.get("/api/UserTraining/Delete/" + id)
             .done(function (result) {
                 filterGrid();
                 showMessage('User Training', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='sortColumnName' type='text'>").val(sortColumnName))
         .append($("<input name='screenName' type='text'>").val("User_Training"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_EngTrainingScheduleTxn_Export"))
