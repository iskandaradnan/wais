$(function () {
    var asetInfoTypes = "Manufacturer:Manufacturer;Model:Model;";
    genarateGrid(asetInfoTypes);
});

function genarateGrid(asetInfoTypes) {
    var isFromSave = false;
    var gridClicked = false;
    var AsetInfoTypes = asetInfoTypes.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/assetinformation/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Type', 'Name', 'Edit', 'Delete', 'View'],
        colModel: [
            { key: true, hidden: true, name: 'AssetInfoId', width: '0%' },
            //{ key: false, search: true, name: 'AssetInfoTypeName', width: '40%', searchoptions: { sopt: ['cn', 'eq', 'ne'] } },
            { key: false, search: true, name: 'AssetInfoTypeName', width: '40%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: AsetInfoTypes, defaultValue: "Manufacturer" } },
            { key: false, name: 'AssetInfoValue', width: '40%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { name: 'edit', search: false, index: 'AssetInfoId', width: '5%', sortable: false, formatter: editLink },
            { name: 'delete', search: false, index: 'AssetInfoId', width: '5%', sortable: false, formatter: deleteLink },
            { name: 'view', search: false, index: 'AssetInfoId', width: '5%', sortable: false, formatter: viewLink },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
        {
            ...
            onClickButton: function () {
                window.location.href = "/bems/assetinformation/add";
            }
        }
        )
        .navButtonAdd('#grid_toppager', {
...
function editLink(cellValue, options, rowObject, action) {
    if (rowObject.AssetInfoTypeName == "Manufacturer") {
        rowObject.AssetInfoId = rowObject.AssetInfoId.toString() + 5;
    }
    else if (rowObject.AssetInfoTypeName == "Make") {
        rowObject.AssetInfoId = rowObject.AssetInfoId.toString() + 6;
    }
    else if (rowObject.AssetInfoTypeName == "Brand") {
        rowObject.AssetInfoId = rowObject.AssetInfoId.toString() + 7;
    }
    else if (rowObject.AssetInfoTypeName == "Model") {
        rowObject.AssetInfoId = rowObject.AssetInfoId.toString() + 8;
    }
    return "<a href='/bems/assetinformation/edit/" + rowObject.AssetInfoId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
       ...
       var AssetInfoTypeName = escapeShowTitle(rowdata.AssetInfoTypeName);
       if (rowdata.AssetInfoValue != null && rowdata.AssetInfoValue != "") {
           var AssetInfoValue = escapeShowTitle(rowdata.AssetInfoValue);
        }
       
       return "<a href='javascript:confirmDelete(" + rowdata.AssetInfoId + ',' + '"' + AssetInfoTypeName + '"' + ',' + '"'
        + AssetInfoValue + '"' + ',' + '"' + gridID + '"'
        + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
}
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/bems/assetinformation/view/" + rowObject.AssetInfoId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function confirmDelete(id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            id = id.toString();
            var AssetInfoTypeName = id.substring(id.length - 1);
            id = id.substring(0, id.length - 1);

            $.get("/api/assetinformation/Delete/" + id + "/" + AssetInfoTypeName)
             .done(function (result) {
                 filterGrid();
                 showMessage('Asset Information', CURD_MESSAGE_STATUS.DS);
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
         ...
         .append($("<input name='screenName' type='text'>").val("Asset_Information"))
         .append($("<input name='spName' type='text'>").val("uspFM_EngAssetStandardizationInformation_Export"))
