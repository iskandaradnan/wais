...
function genarateGrid() {
    ...
    grid.jqGrid({
        ...
        colNames: ['Id', 'Customer Code','Customer Name', 'State','Country'],
        colModel: [
            { key: true, hidden: true, name: 'CustomerId', width: '0%' },
            { key: false, search: true, name: 'CustomerCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'CustomerName', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
           
            { key: false, search: true, name: 'State', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Country', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: false, name: 'ActiveFromDate', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            //{ key: false, search: false, name: 'ActiveToDate', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: false, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
     ...
     //   caption: 'Customer Registration',
       ...
       //         CheckMultipleTabs("/BEMS/Customer/Add");
       //     }
       // }
       // )
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='sortColumnName' type='text'>").val(sortColumnName))
         //.append($("<input name='screenName' type='text'>").val("Customer_Registration"))
         .append($("<input name='screenName' type='text'>").val("Customer_Registration"))
           //.append($("<input name='headerColumnNames' type='text'>").val(headercolumns))
          .append($("<input name='spName' type='text'>").val("uspFM_MstCustomer_Export"))
