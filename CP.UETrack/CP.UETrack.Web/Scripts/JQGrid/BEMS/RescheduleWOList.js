$(function () {
    var ServiceValues = "PPM:PPM;RI:RI;PDM:PDM;Calibration:Calibration;Radiology QC:Radiology QC"
...
function genarateGrid(serviceValues) {
    ...
    grid.jqGrid({
        url: "/api/reschedulewo/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Work Order No.', 'Asset No.', 'Asset Type Description', 'Type of Planner', 'Location Name', 'Assignee'],
        colModel: [
            { key: true, hidden: true, name: 'WorkOrderId', width: '0%' },
            { key: false, search: true, name: 'WorkOrderNo', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetNo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetDescription', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TypeOfWorkOrderName', width: '20%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ServiceValues, defaultValue: "PPM" } },
            { key: false, search: true, name: 'LocationName', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'Assignee', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ key: false, search: true, name: 'MaintenanceDetails', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne'] } },
            //{ name: 'edit', search: false, index: 'WorkOrderId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'WorkOrderReschedulingId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'WorkOrderId', width: '5%', sortable: false, formatter: viewLink },
        ],
        pager: jQuery('#pager'),
        rowNum: 5,
        rowList: [5, 10, 20, 50],
       ...
       // caption: 'Reschedule',
        ...
        //        CheckMultipleTabs("/bems/reschedulewo/add");
        //    }
        //})
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Reschedule"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_EngMwoReschedulingTxn_Export"))
