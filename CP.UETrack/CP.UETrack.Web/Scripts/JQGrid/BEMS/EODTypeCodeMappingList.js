...
function genarateGrid() {
    ...
    grid.jqGrid({
        url: "/api/EODTypeCodeMapping/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ...
        colModel: [
            ...
            { key: false, search: true, name: 'ServiceName', width: '40%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc' ]} },
            { key: false, search: true, name: 'CategorySystemName', width: '45%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc'] } },
        ...
        width: '100%',
        viewrecords: true,
       // caption: 'Category / System Master',
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
        {
            ...
            onClickButton: function () {
                window.location.href = "/bems/eodtypecodemapping/add";
            }
        }
        )
        .navButtonAdd('#grid_toppager', {
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='/bems/eodtypecodemapping/edit/" + rowObject.CategorySystemId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
       ...
       return "<a href='javascript:confirmDelete(" + rowdata.CategorySystemId + ',' + '"' + ServiceName + '"' + ',' + '"'
        + Name + '"' + ',' + '"' + gridID + '"'
        + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
}
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/bems/eodtypecodemapping/view/" + rowObject.CategorySystemId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;' ></a>";
}
...
function confirmDelete(id, code, description, gridId) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/eodtypecodemapping/Delete/" + id)
             ...
             .fail(function (response) {
                 ...
                 if (response.status == 400) {
                     ...
                     showMessage('Category/System Name and TypeCode is', CURD_MESSAGE_STATUS.RAU);
                 }
                 $('#myPleaseWait').modal('hide');
             });
        }
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='sortColumnName' type='text'>").val(sortColumnName))
         .append($("<input name='screenName' type='text'>").val("EOD_Type_Code_Mapping"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_EngEODCategorySystemDet_Export"))
