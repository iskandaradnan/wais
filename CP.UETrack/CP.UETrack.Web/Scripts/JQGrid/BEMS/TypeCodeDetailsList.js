...
function genarateGrid( ) {
    ...
    grid.jqGrid({
        url: "/api/TypeCode/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id', 'Asset Classification Code', 'Asset Classification Description', 'Asset Type Code', 'Asset Type Code Description'],
        colModel: [
            { key: true, hidden: true, name: 'AssetTypeCodeId', width: '0%' },
            { key: false, search: true, name: 'AssetClassificationCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetClassificationDescription', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetTypeCode', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetTypeDescription', width: '30%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            //{ name: 'edit', search: false, index: 'AssetTypeCodeId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'AssetTypeCodeId', width: '5%', sortable: false, formatter: deleteLink },
           // { name: 'view', search: false, index: 'AssetTypeCodeId', width: '5%', sortable: false, formatter: viewLink },
        ...
        width: '100%',
        viewrecords: true,
       // caption: 'Asset Type Code Details',
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='/BEMS/typecodedetails/edit/" + rowObject.AssetTypeCodeId + "' class='glyphicon glyphicon-edit' style='color:#477ca9;font-size: 13px;'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    var Name = escapeShowTitle(rowdata.AssetTypeCode);
    var desc = escapeShowTitle(rowdata.AssetTypeDescription);

    return "<a href='javascript:confirmDelete(" + rowdata.AssetTypeCodeId + ',' + '"' + Name + '"' + ',' + '"'
     + desc + '"' + ',' + '"' + gridID + '"'
     + ")' class='glyphicon glyphicon-remove' style='color:red;font-size: 13px;'></a>";
}
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/BEMS/typecodedetails/view/" + rowObject.AssetTypeCodeId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;'></a>";
}
        ...
        if (result) {
            $.get("/api/TypeCode/Delete/" + id)
             .done(function (result) {
                 filterGrid();
                 showMessage('Type Code Details', CURD_MESSAGE_STATUS.DS);
             ...
             .fail(function () {
                 showMessage('Type Code Details', CURD_MESSAGE_STATUS.DF);
                 $('#myPleaseWait').modal('hide');
             });
        }
    });
...
//        .append($("<input name='screenName' type='text'>").val("Asset_Type_Code_Details"))
//        .append($("<input name='headerColumnNames' type='text'>").val(headercolumns))
//          .append($("<input name='spName' type='text'>").val("uspFM_EngAssetTypeCode_Export"))
