...
function genarateGrid() {
    ...
    grid.jqGrid({
        ...
        colNames: ['Id', 'Work Order No.', 'Asset No.', 'Type of Planner', 'Details', 'Edit', 'Delete', 'View'],
        colModel: [
            { key: true, hidden: true, name: 'WorkOrderId', width: '0%' },
            { key: false, search: true, name: 'WorkOrderNo', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetNo', width: '15%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TypeOfWorkOrderName', width: '20%', searchoptions: { sopt: ['cn', 'eq'] } },
            { key: false, search: true, name: 'MaintenanceDetails', width: '25%', search: false, searchoptions: { sopt: ['cn', 'eq'] } },
    ...
    }).navGrid('#pager', { cloneToTop: true, edit: false, add: false, del: false, search: false, refresh: true },
        {
            ...
            onClickButton: function () {
                CheckMultipleTabs("/bems/scheduledwo/add");
            }
        }
        )
        .navButtonAdd('#grid_toppager', {
...
function editLink(cellValue, options, rowObject, action) {
    ...
    return "<a href='#' onclick=CheckMultipleTabs('/bems/scheduledwo/Edit/" + rowObject.WorkOrderId + "') class='ui-icon ui-icon-pencil'></a>";
}
...
function deleteLink(cellValue, options, rowdata, action) {
    ...
    return "<a href='javascript:confirmDelete(" + rowdata.WorkOrderId + ',' + '"' + rowdata.WorkOrderNo + '"' + ',' + '"'
...
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='#' onclick=CheckMultipleTabs('/bems/scheduledwo/View/" + rowObject.WorkOrderId + "') class='glyphicon glyphicon-eye-open'></a>";
}
...
function Export(exportType) {
    ...
    var headercolumns = ['Name', 'User Type', 'Status'];
    var columncolumns = ['Name', 'UserTypeName', 'Status'];
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          ...
          .append($("<input name='viewModelName' type='text'>").val("UmUserRoleExportData"))
