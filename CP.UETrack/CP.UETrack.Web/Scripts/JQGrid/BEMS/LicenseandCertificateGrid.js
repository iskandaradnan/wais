$(function () {
    var CategoryValues = "Asset:Asset;Company:Company;Personnel:Personnel;";
    var StatusValues = "Active:Active;Inactive:Inactive;";
    genarateGrid(CategoryValues, StatusValues);
});

function genarateGrid(categoryValues, statusValues) {
    var isFromSave = false;
    var gridClicked = false;
    CategoryValues = categoryValues.replace(/[,;]$/, '').toString();   
    StatusValues = statusValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/LicenseAndCertificateApi/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['Id','Asset No.', 'Asset Description', 'License No.', 'Expiry Date', 'Status', 'Category','Issuing Body','Class / Grade' ],
        colModel: [
            { key: true, hidden: true, name: 'LicenseId', width: '0%' },
            { key: false, name: 'AssetNo', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'AssetDescription', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'LicenseNo', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'ExpiryDate', width: '15%', formatoptions: { srcformat: "d-M-Y H:i", newformat: "d-M-Y" }, formatter: "date", resizable: true, searchoptions: { dataInit: function (elem) { $(elem).datetimepicker({ timepicker: false, format: 'd-M-Y', step: 15, scrollInput: false }) }, sopt: ['eq', 'ne', 'lt', 'gt', 'le', 'ge'] } },
            { key: false, name: 'StatusVal', width: '25%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: StatusValues, defaultValue: "Active" } },
            { key: false, search: true, name: 'CategoryVal', width: '25%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: CategoryValues, defaultValue: "Asset" } },
            { key: false, name: 'IssuingBodyVal', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, name: 'ClassGrade', width: '25%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
           
            //{ name: 'edit', search: false, index: 'LicenseId', width: '5%', sortable: false, formatter: editLink },
            //{ name: 'delete', search: false, index: 'LicenseId', width: '5%', sortable: false, formatter: deleteLink },
            //{ name: 'view', search: false, index: 'LicenseId', width: '5%', sortable: false, formatter: viewLink },
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("License_and_Certificate_Details"))
          .append($("<input name='spName' type='text'>").val("uspFM_EngLicenseandCertificateTxn_Export"))
