$(function () {
    var ServiceValues = "Recall:Recall;Alert:Alert;";
    var WrkStatusValues = "Cancelled:Cancelled;Closed:Closed;Completed:Completed;Open:Open;Reassigned:Reassigned;Work In Progress:Work In Progress;";
    genarateGrid(ServiceValues, WrkStatusValues);
});

function genarateGrid(serviceValues, wrkStatusValues) {
    var isFromSave = false;
    var gridClicked = false;
    var ServiceValues = serviceValues.replace(/[,;]$/, '').toString();
    var WrkStatusValues = wrkStatusValues.replace(/[,;]$/, '').toString();
    var grid = $("#grid"),
    prmSearch = { multipleSearch: true, overlay: false };
    grid.jqGrid({
        url: "/api/CRMWorkorderTab/GetAll",
        datatype: 'json',
        mtype: 'Get',
        ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },
        colNames: ['CRMRequestWOId', 'Work Order No.','Request No.', 'Asset No.', 'Model', 'Manufacturer', 'Staff Assigned', 'Request Type', 'Work Order Status'],
        colModel: [
            { key: true, hidden: true, name: 'CRMRequestWOId', width: '0%' },
            { key: false, search: true, name: 'CRMRequestWONo', width: '20%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'RequestNo', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'AssetNo', width: '17%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            ...
            { key: false, search: true, name: 'StaffName', width: '10%', searchoptions: { sopt: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'] } },
            { key: false, search: true, name: 'TypeOfRequest', width: '8%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: ServiceValues, defaultValue: "Incident" } },
            { key: false, search: true, name: 'WorkOrderStatus', width: '10%', stype: "select", searchoptions: { sopt: ["eq", "ne"], value: WrkStatusValues, defaultValue: "Cancelled" } },
...
function viewLink(cellValue, options, rowObject, action) {
    return "<a href='/bems/CRMWorkorder/view/" + rowObject.CRMRequestWOId + "' class='glyphicon glyphicon-eye-open' style='color:#477ca9;font-size: 13px;' ></a>";
}
...
function Export(exportType) {
    ...
    var $downloadForm = $("<form method='POST'>")
        ...
        .append($("<input name='screenName' type='text'>").val("Work_Order"))
        .append($("<input name='screenName' type='text'>").val(screenTitle.split(" ").join('_')))
          .append($("<input name='spName' type='text'>").val("uspFM_CRMRequestWorkOrderTxn_Export"))
