...
$(document).ready(function () {
    ...
    formInputValidation("CRMUsrTraiAttachmentTabForm");

    $("#CRMsrTraiAttachtab").click(function () {
                ...
                $.get("/api/Document/getAttachmentDetails/" + HiddenId)
                  .done(function (result) {
                      ...
                      if (getResult != null && getResult.FileUploadList != null && getResult.FileUploadList.length > 0) {
                          ...
                          AddNewRowfileupload();
                          var SchNo = $('#UserTrainComptraiSchNo').val();
                          var ActDate = $('#UserTrainCompActDate').val();

                          $("#UserTrainComptraiSchNoAtt").val(SchNo).prop("disabled", "disabled");
                          $("#UserTrainCompActDateAtt").val(ActDate).prop("disabled", "disabled");
    ...
    $('#btnEditAttachment,#btnSaveAttachment').click(function () {
        ...
        var isFormValid = formInputValidation("CRMUsrTraiAttachmentTabForm", 'save');
        if (!isFormValid) {
...
function BindGetByIdVal(getResult) {
    ...
    $.each(getResult.FileUploadList, function (ind, data) {
        ...
        if (ActionType == "View") {
            $("#CRMUsrTraiAttachmentTabForm :input:not(:button)").prop("disabled", true);
