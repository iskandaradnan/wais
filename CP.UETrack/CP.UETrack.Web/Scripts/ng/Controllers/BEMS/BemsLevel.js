$(document).ready(function () {
    ...
    formInputValidation("formBemsLevel");
    $('#btnDelete').hide();
    $('#btnEdit').hide();
  ...
  .fail(function () {
    ...
    function getBlockList(levelFacilityId) {
       ...
       if (levelFacilityId !== "null") {
            $.get("/api/Level/GetBlockData/" + levelFacilityId)
            .done(function (result) {
                var loadResult = JSON.parse(result);
    ...
    $("#btnSave, #btnEdit,#btnSaveandAddNew").click(function () {
        ...
        var isFormValid = formInputValidation("formBemsLevel", 'save');
        if (!isFormValid) {
        ...
        var jqxhr = $.post("/api/Level/Save", MstLevel, function (response) {
...
function LinkClicked(id) {
    ...
    $("#formBemsLevel :input:not(:button)").parent().removeClass('has-error');
    $("div.errormsgcenter").text("");
    $('#errorMsg').css('visibility', 'hidden');
    $('.nav-tabs a:first').tab('show');
    var action = "";
...
$("#btnNextScreenSave").click(function () {
    ...
    if (hdnStatus == 0 || hdnStatus == '0') {
        ...
        bootbox.confirm(msg, function (Conform) {
            if (Conform) {
                window.location.href = "/bems/userarea/Add/" + primaryId;
            }
            else {
                bootbox.hideAll();
                return false;
