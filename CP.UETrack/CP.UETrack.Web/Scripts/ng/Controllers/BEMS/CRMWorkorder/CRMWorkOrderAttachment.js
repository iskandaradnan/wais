...
$(document).ready(function () {
    ...
    formInputValidation("CRMWrlOrdAttachmentTabForm");

    $("#CRMWrkOrdAttachtab").click(function () {
                ...
                $.get("/api/Document/getAttachmentDetails/" + HiddenId)
                  .done(function (result) {
                      ...
                      if (getResult != null && getResult.FileUploadList != null && getResult.FileUploadList.length > 0) {
                          ...
                          var WorkOrdDat = $('#crmWorkWorkDat').val();

                          $("#crmWorkWorkOrdNoAtt").val(WorkOrdNo).prop("disabled", "disabled");
                          $("#crmWorkWorkDatAtt").val(WorkOrdDat).prop("disabled", "disabled");
    ...
    $('#btnEditAttachment,#btnSaveAttachment').click(function () {
        ...
        var isFormValid = formInputValidation("CRMWrlOrdAttachmentTabForm", 'save');
        if (!isFormValid) {
...
function BindGetByIdVal(getResult) {
    ...
    $.each(getResult.FileUploadList, function (ind, data) {
        ...
        if (ActionType == "View") {
            $("#CRMWrlOrdAttachmentTabForm :input:not(:button)").prop("disabled", true);
