...
$(document).ready(function () {
    ...
    formInputValidation("CRMReqAttachmentTabForm");

    $("#CRMreqAttachtab").click(function () {
                ...
                $.get("/api/Document/getAttachmentDetails/" + HiddenId)
                  .done(function (result) {
                      ...
                      if (getResult != null && getResult.FileUploadList != null && getResult.FileUploadList.length > 0) {
                          ...
                          var reqdate = $('#RequestDateTime').val();

                          $("#RequestNoAtt").val(reqno).prop("disabled", "disabled");
                          $("#RequestDateTimeAtt").val(reqdate).prop("disabled", "disabled");
    ...
    $('#btnEditAttachment,#btnSaveAttachment').click(function () {
        ...
        var isFormValid = formInputValidation("CRMReqAttachmentTabForm", 'save');
        if (!isFormValid) {
    ...
    $('#btnAttachmentCancel').click(function () {
        window.location.href = "/bems/CRMRequest";
    });
...
function BindGetByIdVal(getResult) {
    ...
    $.each(getResult.FileUploadList, function (ind, data) {
        ...
        if (ActionType == "View") {
            $("#CRMReqAttachmentTabForm :input:not(:button)").prop("disabled", true);
