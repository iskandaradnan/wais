...
$('#liAFAdditionalInfo').click(function () {
    ...
    if (primaryId == 0 || primaryId == undefined) {
        bootbox.alert("Scheduled Work Order must be Saved before entering additional information");
        return false;
    }   
    ...
    $.get("/api/additionalFields/LoadAdditionalInfo/" + 337)
     .done(function (result) {
         ...
         if (status == 'Closed') {
             ...
             $("#frmAdditionalInfo :input:not(:button)").prop("disabled", false);
             $('#btnAdditionalInfoEdit').show();
         }
...
$("#btnAdditionalInfoCancel").click(function () {   
    window.location.replace("/bems/scheduledworkorder");
});
