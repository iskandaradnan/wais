$(document).ready(function () {
    ...
    formInputValidation("formFemsBlock");
    $('#btnDelete').hide();
    $('#btnEdit').hide();
    $('#btnNextScreenSave').hide();   
    $.get("/api/FemsBlock/Load")
  ...
  .fail(function () {
    ...
    $("#btnSave, #btnEdit,#btnSaveandAddNew").click(function () {
        ...
        var isFormValid = formInputValidation("formFemsBlock", 'save');
        if (!isFormValid)
        {
        ...
        var jqxhr = $.post("/api/FemsBlock/Save", MstBlock, function (response) {
...
function MasterLevelLinkClicked(id) {
    ...
    $("#formFemsBlock :input:not(:button)").parent().removeClass('has-error');
    $("div.errormsgcenter").text("");
    $('#errorMsg').css('visibility', 'hidden');
    var action = "";
    $('#primaryID').val(id);
    ...
    if (action == 'View') {
        $("#formFemsBlock :input:not(:button)").prop("disabled", true);
    } else {
        $('#btnEdit').show();
        $('#btnSave').hide();
    ...
    if (primaryId != null && primaryId != "0") {
        $.get("/api/FemsBlock/Get/" + primaryId)
          .done(function (result) {
...
$("#btnNextScreenSave").click(function () {
    ...
    else if (primaryId != null && primaryId != 0 && primaryId != "0" && primaryId != '')
    {
        ...
        bootbox.confirm(msg, function (Conform) {
            if (Conform) {
                window.location.href = "/fems/FemsLevel/Add/" + primaryId;
            }
            else {
                bootbox.hideAll();
                return false; 
