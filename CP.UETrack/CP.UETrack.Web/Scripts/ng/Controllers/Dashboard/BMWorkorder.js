...
$(document).ready(function () {
    ...
    $.get("/api/Dashboard/LoadWorkorderChart/" + StartYear + "/" + StartMonth + "/" + EndYear + "/" + EndMonth + "/" + FacilityId + "/" + UserId)
...
function WorkOrdChartdataOptions(getResult) {
    ...
    for (ind = 0; ind < getResult.BMWorkorderChartData.length; ind++) {
        workOrdStstus = getResult.BMWorkorderChartData[ind].WorkOrdStatus;
        WorkorderCount = getResult.BMWorkorderChartData[ind].Count;
         perc = getResult.BMWorkorderChartData[ind].Percentage;
        Col = ChartColor(ind);
