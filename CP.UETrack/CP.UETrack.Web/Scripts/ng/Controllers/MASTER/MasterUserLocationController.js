$(document).ready(function () {
    ...
    var UserAreaId = $("#UserAreaId").val();
    if (UserAreaId != null && UserAreaId != '' && UserAreaId != 0 && UserAreaId != "0") {
        $.get("/api/MasterUserLocation/Load/" + UserAreaId)
 ...
 .fail(function () {
    ...
    $("#btnSave,#btnEdit,#btnSaveandAddNew").click(function () {
        ...
        if (!isFormValid) {
            ...
            $('#errorMsg').css('visibility', 'visible');
            $('#myPleaseWait').modal('hide');
            $('#btnmulSave').attr('disabled', false);
            return false;
        }
        ...
        var jqxhr = $.post("/api/MasterUserLocation/Add", Obj, function (response) {
            ...
            $('#btnmulSave').attr('disabled', false);
            $('#myPleaseWait').modal('hide');
            if (CurrentbtnID == "btnmulSaveandAddNew") {
                ClearFields();
            }
            ClearFields();
        },
    ...
    $('#btnmulAddNew').click(function () {
        window.location.reload();
    });

    $("#btnmulCancel").click(function () {
...
function LinkClicked(id) {
    ...
    if (primaryId != null && primaryId != "0") {
        $.get("/api/MasterUserLocation/Get/" + primaryId)
          .done(function (result) {
...
function confirmDelete(ID) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/MasterUserLocation/Delete/" + ID)
             .done(function (result) {
                 filterGrid();
                 $(".content").scrollTop(0);
