...
$(document).ready(function () {
    ...
    var LevelId = $("#LevelId").val();
    if (LevelId != null && LevelId != '' && LevelId != 0 && LevelId != "0") {
        $.get("/api/MasterUserArea/Load/" + LevelId)
 ...
 .fail(function () {
    ...
    $("#btnSave, #btnEdit,#btnSaveandAddNew").click(function () {
        ...
        var jqxhr = $.post("/api/MasterUserArea/Add", Obj, function (response) {
...
function LinkClicked(id) {
    ...
    if (primaryId != null && primaryId != "0") {
        $.get("/api/MasterUserArea/Get/" + primaryId)
          .done(function (result) {
              ...
              $('#Category').val(getResult.Category);
              if (getResult.Active) {
                  $('#Active').val(1);
                  $("#ActiveToDate").prop('disabled', true);
                  //$("#stopdatelabelid").html('Stop Service Date <span class="red">&nbsp;*</span>');
...
function confirmDelete(ID) {
    ...
    bootbox.confirm(message, function (result) {
        if (result) {
            $.get("/api/MasterUserArea/Delete/" + ID)
             .done(function (result) {
                 filterGrid();
                 $(".content").scrollTop(0);
...
$("#btnNextScreenSave").click(function () {
    ...
    if (hdnStatus == 0 || hdnStatus == '0' || hdnStatus == 'null') {
        ...
        bootbox.confirm(msg, function (Conform) {
            if (Conform) {
                window.location.href = "/master/Masteruserlocation/Add/" + primaryId;
            }
            else {
                bootbox.hideAll();
                return false;
