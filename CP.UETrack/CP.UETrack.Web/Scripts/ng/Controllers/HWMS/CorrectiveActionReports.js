...
$(document).ready(function () {
    ...
    $.get("/api/CLSCorrectiveActionReport/Load")
        .done(function (result) {
            ...
            Indicatorsvalue = "<option value='' Selected>" + "Select" + "</option>";
            FileTypeValues = "<option value='' Selected>" + "Select" + "</option>";
            priorityvalues= "<option value='' Selected>" + "Select" + "</option>";
            ResponsibilityValues="<option value='' Selected>" + "Select" + "</option>";
            ...
            $("#txtResponsibility1").html(ResponsibilityValues);
            ...
            $("#ddlFileType1").html(FileTypeValues);

            for (var i = 0; i < loadResult.IndicatorValues.length; i++) {

                Indicatorsvalue += "<option value=" + loadResult.IndicatorValues[i].LovId + ">" + loadResult.IndicatorValues[i].FieldValue + "</option>"

            }
            $("#txtIndicator").html(Indicatorsvalue);
            AutogeneratedCode();
    ...
    $('#txtFUC').on('input propertychange paste keyup', function (event) {
        DisplayFetchResult('divFetch8', followupCARFetchObj, "/api/CorrectiveActionReports/FollowUpCARNoFetch", "UlFetch8", event, 1);//1 -- pageIndex
    });
    ...
    $(body).on('input propertychange paste keyup', '.responsiblepersoncode', function (event) {
    ...
    $('.Indicator').on('change', function () {
        var value = $('#txtIndicators option:selected').text();
        if (value !== "Select") {
    ...
    $("#btnSave, #btnSaveandAddNew").click(function () {
        ...
        $("#CAR tbody tr").each(function () {
            ...
            var DWRObj = {};
            DWRObj.CARActivityId = $(this).find("[id^=hdnactivityId]")[0].value;
            DWRObj.Activity = $(this).find("[id^=txtActivity]")[0].value;
            DWRObj.Startdate = $(this).find("[id^=txtSD]")[0].value;
            DWRObj.TargetDate = $(this).find("[id^=txtTD]")[0].value;
            DWRObj.Actualcompletiondate = $(this).find("[id^=txtACD]")[0].value;
            DWRObj.Responsibility = $(this).find("[id^=txtResponsibility]")[0].value;
            DWRObj.Responsibleperson = $(this).find("[id^=txtResponsible]")[0].value;
            DWRObj.isDeleted = $(this).find('input:checkbox[id^=isDelete]').prop("checked");

            obj.CARActivityList.push(DWRObj);

        })
            
        $.post("/api/CorrectiveActionReports/Save", obj, function (response) {
            ...
            callback: function (result) {
                if (result) {
                    if ($("input[type='checkbox']:checked").length > 0) {
                        $("#activitytbody ").find('input[name="isDelete"]').each(function () {
                            if ($(this).is(":checked")) {
    ...
    $("#btnSaveAttachment").click(function () {
        ...
        $.post("/api/CorrectiveActionReports/AttachmentSave", obj, function (response) {
...
function EmptyFields() {
    ...
    $("#primaryID").val(0);
    var i = 1;
    $("#activitytbody").find('tr').each(function () {
        if (i > 1) {
            $(this).remove();
        }
        i += 1;
...
function LinkClicked(id) {
    ...
    if (primaryId != null && primaryId != "0") {
        $.get("/api/CorrectiveActionReports/Get/" + primaryId)
            .done(function (result) {
...
function disableFields(isDisable) {
    ...
    if (isDisable == 'True') {
        ...
        $("#txtIndicator").prop("disabled", true);
        ...
        $("#txtIndicator").prop("disabled", false);
...
function fillDetails(result) {
    ...
    if (result != null) {
        ...
        $("#txtIndicator").val(result.Indicator);
        ...
        fillAttachment(result.CARAttachmentList);

        var rc = $("#txtIndicator option:selected").text();
        ...
        $("#tbodyHistory").html('');
        addhistorydetails();
        var CARStatus = $("#txtStatus option:Selected").text();

        $('#txtCARStatus1').val(CARStatus);
...
function fillAttachment(result) {
    ...
    if (result != null) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            $('#hdnAttachmentId' + rowNum2).val(result[i].AttachmentId);
            $('#ddlFileType' + rowNum2).val(result[i].FileType);
            $('#txtFileName' + rowNum2).val(result[i].FileName);
            $('#fileAttachment' + rowNum2).val(result[i].AttachmentName);
...
function AddActivity(num) {
    ...
    var CheckBox = '<td style="text-align:center">   <input type="hidden" id="hdnactivityId' + num + '" value="0" /> <input type="checkbox" id="isDelete' + num+'" name="isDelete"></td>';
    ...
    var ActualCompletion = '<td> <input type="text" class="form-control" disabled="disabled" id="txtACD' + num + '" autocomplete="off"  name="ActualCompletion" maxlength="25"  /></td>';
    ...
    var ResponsiblePerson = '<td> <input type="text" class="form-control responsiblepersoncode" placeholder="please select" id="txtResponsible' + num + '" autocomplete="off"  name="ResposiblePerson" maxlength="25"  /><input type = "hidden" id="hdnresponsibleId' + num + '" />   <div class="col-sm-12" id="divFetchId' + num + '">   </div></td>';

    $("#activitytbody ").append('<tr>' + CheckBox + activity + startdate + TargetDate + ActualCompletion + Responsibility + ResponsiblePerson + '</tr>');
...
function AutogeneratedCode() {
    ...
    $.get("/api/CorrectiveActionReports/AutoGeneratedCode", function (response) {
...
function addhistorydetails() {
    ...
    $("#tableHistory tbody").append('<tr>' + StatusValue + RootCause + Solution + Remarks + '</tr>');
