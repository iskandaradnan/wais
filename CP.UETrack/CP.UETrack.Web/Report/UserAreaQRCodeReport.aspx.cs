...
{
    public partial class UserAreaQRCodeReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    reportViewer.LocalReport.ReportPath = Server.MapPath(Request.ApplicationPath) + @"RDLCReports\QR_Code_UserArea.rdlc";
                    var datasource = new ReportDataSource("DataSet1", dt); //UserAreaQRCode DataSet2
                    reportViewer.LocalReport.DataSources.Add(datasource);
                    reportViewer.LocalReport.Refresh();
                    reportViewer.LocalReport.DisplayName = "Area_QR_Code_" + CurrentDate;
                }
            }
            catch (Exception ex)
            {
        ...
        private DataTable GetSPResult()
        {
            ...
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["UETrackConnectionString"].ConnectionString))
            {
                try
                {
                    using (var cmd = new SqlCommand("uspFM_UserAreaQRCode_Report", conn))
                    {
