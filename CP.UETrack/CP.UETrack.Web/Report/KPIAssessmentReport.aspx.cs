...
{
    public partial class KPIAssessmentReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    reportViewer.LocalReport.ReportPath = Server.MapPath(Request.ApplicationPath) + @"RdlReports\LatestReportsRDLC\KPIAssessmentReport.rdlc";
                    var datasource = new ReportDataSource("usp_KPIAssessmentReport", GetSPResult("usp_KPIAssessmentReport", Year, Month));
                ...
                try
                {
                    ...
                    using (SqlConnection con = new SqlConnection(dbAccessDAL.ConnectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            ...
                            switch (Spname)
                            {
                                case "usp_KPIAssessmentReport":
                                    cmd.CommandText = "usp_KPIAssessmentReport";
