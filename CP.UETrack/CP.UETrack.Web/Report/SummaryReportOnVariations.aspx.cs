...
{
    public partial class SummaryReportOnVariations : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    reportViewer.LocalReport.ReportPath = Server.MapPath(Request.ApplicationPath) + @"RdlReports\LatestReportsRDLC\VMSummary_L1.rdlc";
                    var datasource = new ReportDataSource("DataSet1", GetSPResult("uspFM_VMSummaryEquipmentBEMSRpt_L1",
                         VOVFromYear, VOVFromMonth, VOVToYear, VOVTomonth));
        ...
        protected void reportViewer_Drillthrough(object sender, DrillthroughEventArgs e)
        {
            ...
            var datasource = new ReportDataSource("uspFM_VMSummaryEquipmentBEMSRpt_L2", GetSPResult("uspFM_VMSummaryEquipmentBEMSRpt_L2", VOVFromYear, VOVFromMonth, VOVToYear, VOVTomonth, month, Year));
                ...
                try
                {
                    ...
                    using (SqlConnection con = new SqlConnection(dbAccessDAL.ConnectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            ...
                            switch (Spname)
                            {
                                case "uspFM_VMSummaryEquipmentBEMSRpt_L1":
                                    cmd.CommandText = "uspFM_VMSummaryEquipmentBEMSRpt_L1";
                                    ...
                                    break;
                                case "uspFM_VMSummaryEquipmentBEMSRpt_L2":
                                    cmd.CommandText = "uspFM_VMSummaryEquipmentBEMSRpt_L2";
                                    cmd.Parameters.AddWithValue("@FacilityId", userDetails.FacilityId);
