    ...
    public class UserTrainingDAL : IUserTrainingDAL
    {
        ...
        public UserTrainingDropdown Load()
        {
            try
            {
                ...
                UserTrainingDropdown usertraingddn = new UserTrainingDropdown();
                ...
                if (_UserSession.UserTypeId==1)
                {
                    usertrainingdropdownvalues.TrainingTypeLovs.RemoveAll(x => x.LovId == 254);
                }
                else
                {
                }
                ...
                using (SqlConnection con = new SqlConnection(dbAccessDAL.ConnectionString))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        if (ds1.Tables.Count != 0)
                        {
                            usertraingddn.RequestServiceList = dbAccessDAL.GetLovRecords(ds1.Tables[0], "FacilityId");
                            //CORMDropdownList.RequestServiceList = dbAccessDAL.GetLovRecords(ds1.Tables[0], "FacilityId");
                    ...
                    {
                        lv.Add(new LovValue() { LovId = 3, FieldValue = "CLS", DefaultValue = false });
                ...
                usertrainingdropdownvalues.RequestServiceList = lv;
        ...
        public UserTrainingCompletion Save(UserTrainingCompletion usrtraing, out string ErrorMessage)
        {
            ...
            string services = "";
            UserTrainingCompletion usrtraingComp = new UserTrainingCompletion();
         //   usrtraingComp = GET_ServiceAndCustomer_ID(usrtraing.TypeOfServiceRequest, _UserSession.CustomerId, _UserSession.FacilityId);
            try
            {

                if (usrtraing.ServiceId == 2)
                {
                    services = "BEMS";
                }
                else
                ...
                parameters.Add("@pServiceId", Convert.ToString(usrtraing.ServiceId));
                ...
                //------------------------------------------------------
                DataTable dt1 = dbAccessDAL.GetMASTERDataTable("uspFM_EngTrainingScheduleTxn_Save", parameters, DataSetparameters);

                #region Commented code
                ////DataTable dtt1 = new DataTable();
                //////----creating individual Models

                ////UserTrainingCompletion FEMSusrtraing = usrtraing;
                ////UserTrainingCompletion BEMSusrtraing = usrtraing;
                ...
                ////dt2.Clear();
                ////if (usrtraing.TypeOfServiceRequest == 2)
                ////{
                ////    foreach (var i in usrtraing.UserTrainingAreaGridData)
                ////    {

                ////        dt2.Rows.Add(i.TrainingScheduleAreaId, i.UserAreaId, null, _UserSession.UserId);
                ////        //bs = get_userAret_Useeas.Gr_Areaids(Convert.ToInt32(userAreas));
                ////        //dt2.Rows[inc]["UserAreaId"];
                ////    }
                ////}
                ////else
                ////{
                ...
                ////{
                ////    services = "BEMS";
                ////    parameters["CustomerId"] = com.BEMS_B_ID.ToString();
                ////    parameters["FacilityId"] = facilities.BEMS_B_ID.ToString();
                ////    var BdbAccessDAL = new BEMSDBAccessDAL();
                ////    dtt1 = BdbAccessDAL.BEMSGetDataTable("uspFM_EngTrainingScheduleTxn_Save", parameters, DataSetparameters);
                ////}
                ////else
                ////{
                ////    services = "FEMS";
                ////    parameters["CustomerId"] = com.FEMS_B_ID.ToString();
                ////    parameters["FacilityId"] = facilities.FEMS_B_ID.ToString();

                ////    var FdbAccessDAL = new FEMSDBAccessDAL();
                ////    dtt1 = FdbAccessDAL.FEMSGetDataTable("uspFM_EngTrainingScheduleTxn_Save", parameters, DataSetparameters);
        ...
        public UserTrainingCompletion Get(int Id, int pagesize, int pageindex)
        {
            ...
            try
            {
                ...
                DataSet ds = dbAccessDAL.MasterGetDataSet("UspFM_EngTrainingScheduleTxn_GetById", parameters, DataSetparameters);
                if (ds != null)
                {
                    if (ds.Tables[0] != null)
                    {
                        entity.CustomerId = Convert.ToInt32(ds.Tables[0].Rows[0]["CustomerId"]);
        ...
        public UserTrainingCompletion GET_ServiceAndCustomer_ID(int Service_ID, int CustomerId, int FacilityId)
        {
            ///-------------Get Modules FEMS OR BEMS Block IDS to update


            UserTrainingCompletion Get_Service_FacilityandCustomer = new UserTrainingCompletion();
            var dss = new DataSet();
