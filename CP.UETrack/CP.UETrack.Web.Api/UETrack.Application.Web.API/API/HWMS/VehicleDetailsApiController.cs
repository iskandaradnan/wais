...
namespace UETrack.Application.Web.API.API.HWMS
{
    ...
    {
        IVehicleDetailsBAL _vehicleDetailsBAL;

        private readonly string _FileName = nameof(HWMSVehicleDetailsApiController);
        private readonly ICommonBAL _commonBAL;

        public HWMSVehicleDetailsApiController(IVehicleDetailsBAL vehicleDetailsBAL, ICommonBAL common)
        {
            _vehicleDetailsBAL = vehicleDetailsBAL;
            _commonBAL = common;
        }
        [HttpGet]
        [Route(nameof(Load))]
        public HttpResponseMessage Load()
        {
            try
            {
                ...
                var result = _vehicleDetailsBAL.Load();
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(VehicleDetails model)
        {
            try
            {
                var ErrorMessage = string.Empty;
                var result = _vehicleDetailsBAL.Save(model, out ErrorMessage);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<VehicleDetails>(Request.GetQueryNameValuePairs());
                var commonDAL = new CommonDAL();
                paginationFilter = commonDAL.GetProperPaginationFilter(paginationFilter);


                var result = _vehicleDetailsBAL.GetAll(paginationFilter);
        ...
        public HttpResponseMessage Get(int Id)
        {
            ...
            try
            {
                ...
                var result = _vehicleDetailsBAL.Get(Id);
                var serialisedData = JsonConvert.SerializeObject(result);
                responseObject = serialisedData == null ? BuildResponseObject(HttpStatusCode.NotFound) : BuildResponseObject(HttpStatusCode.OK, serialisedData);
        ...
        public HttpResponseMessage DescriptionData(string LicenseCode)
        {

            try
            {
                Log4NetLogger.LogEntry(_FileName, nameof(DescriptionData), Level.Info.ToString());
                var result = _vehicleDetailsBAL.DescriptionData(LicenseCode);
                var serialisedData = JsonConvert.SerializeObject(result);
                responseObject = serialisedData == null ? BuildResponseObject(HttpStatusCode.NotFound) : BuildResponseObject(HttpStatusCode.OK, serialisedData);
        ...
        public HttpResponseMessage LicenseCodeFetch(LicCodeDes SearchObject)
        {
            ...
            try
            {
                ...
                var ErrorMessage = string.Empty;
                var result = _vehicleDetailsBAL.LicenseCodeFetch(SearchObject);
                if (result == null)
                {
                    responseObject = BuildResponseObject(HttpStatusCode.NotFound);
                }
