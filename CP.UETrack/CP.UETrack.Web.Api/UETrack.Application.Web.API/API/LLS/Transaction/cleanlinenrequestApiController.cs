...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        // GET: CleanLinenRequestApi
        private readonly ICleanLinenRequestBAL _bal;
        private readonly string fileName = nameof(CleanLinenRequestApiController);

        public CleanLinenRequestApiController(ICleanLinenRequestBAL bal)
        {
            _bal = bal;
        }
        [HttpGet]
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(CleanLinenRequestModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (result == null || result.CleanLinenRequestId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.CleanLinenRequestId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<CleanLinenRequestModel>(Request.GetQueryNameValuePairs());
