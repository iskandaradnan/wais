...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    { 
        // GET: LinenRejectReplacementApi
        private readonly ILinenRejectReplacementBAL _bal;
        private readonly string fileName = nameof(LinenRejectReplacementApiController);


        public LinenRejectReplacementApiController(ILinenRejectReplacementBAL bal)
        {
            _bal = bal;
        }
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(LinenRejectReplacementModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (result == null || result.LinenRejectReplacementId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.LinenRejectReplacementId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<LinenRejectReplacementModel_Filter>(Request.GetQueryNameValuePairs());
