...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        private readonly ILinenCondemnationBAL _bal;
        private readonly string fileName = nameof(LinenCondemnationApiController);


        public LinenCondemnationApiController(ILinenCondemnationBAL bal)
        {
            _bal = bal;
        }
        ...
        public HttpResponseMessage Save(LinenConemnationModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (result == null || result.LinenCondemnationId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.LinenCondemnationId);
