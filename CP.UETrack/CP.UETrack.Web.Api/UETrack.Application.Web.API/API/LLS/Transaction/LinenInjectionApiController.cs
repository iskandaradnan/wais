...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        private readonly ILinenInjectionBAL _bal;
        private readonly string fileName = nameof(LinenInjectionApiController);


        public LinenInjectionApiController(ILinenInjectionBAL bal)
        {
            _bal = bal;
        }
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(LinenConemnationModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (result == null || result.LinenInjectionId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.LinenInjectionId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<LinenConemnationModel>(Request.GetQueryNameValuePairs());
