...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        // GET: LinenInventoryApi
        private readonly ILinenInventoryBAL _bal;
        private readonly string fileName = nameof(LinenInventoryApiController);
        public LinenInventoryApiController(ILinenInventoryBAL bal)
        {
            _bal = bal;
        }
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(TestModel model)
        {
            try
            {
                ...
                {
                    if (result == null || result.LinenInventoryId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.LinenInventoryId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<TestModel>(Request.GetQueryNameValuePairs());
