...
using CP.UETrack.BLL.BusinessAccess.Implementations.LLS.Transaction;
...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        //ICleanLinenDespatchBAL _bal;
        ////private readonly ICommonBAL _common;
        //private readonly string fileName = nameof(CleanlinenDespatchApiController);
        //readonly UserDetailsModel _UserSession = new SessionHelper().UserSession();
        //public CleanlinenDespatchApiController(ICleanLinenDespatchBAL bal, ICommonBAL common)
        //{
        //    _bal = bal;
        //    //_common = common;
        //}

        private readonly ICleanLinenDespatchBAL _bal;
        private readonly string fileName = nameof(CleanlinenDespatchApiController);


        public CleanlinenDespatchApiController(ICleanLinenDespatchBAL bal)
        {
            _bal = bal;
        }
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(CleanLinenDespatchModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (result == null || result.CleanLinenDespatchId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.CleanLinenDespatchId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<CleanLinenDespatchModel>(Request.GetQueryNameValuePairs());
