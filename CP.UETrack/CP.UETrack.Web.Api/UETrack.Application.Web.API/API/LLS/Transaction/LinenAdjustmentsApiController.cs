...
namespace UETrack.Application.Web.API.API.LLS.Transaction
{
    ...
    {
        // GET: LinenAdjustmentsApi
        private readonly ILinenAdjustmentsBAL _bal;
        private readonly string fileName = nameof(LinenAdjustmentsApiController);


        public LinenAdjustmentsApiController(ILinenAdjustmentsBAL bal)
        {
            _bal = bal;
        }
        ...
        [HttpPost]
        [Route(nameof(Save))]
        public HttpResponseMessage Save(LinenAdjustmentsModel model)
        {
            try
            {
                ...
                {
                    if (result == null || result.LinenAdjustmentId == 0)
                        BuildResponseObject(false, resxFileHelper.GetErrorMessagesFromResource("CreateFailed"));
                    else
                    {
                        var ResultData = _bal.Get(result.LinenAdjustmentId);
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<LinenAdjustmentsModel>(Request.GetQueryNameValuePairs());
