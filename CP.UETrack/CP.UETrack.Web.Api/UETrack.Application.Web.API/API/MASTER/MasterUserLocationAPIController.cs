...
{
    [RoutePrefix("api/MasterUserLocation")]
    [WebApiAudit]
    public class MasterUserLocationAPIController : BaseApiController
    {
        private readonly IMasterUserLocationBAL _userLocationBAL;
        private readonly string fileName = nameof(MasterUserLocationAPIController);

        public MasterUserLocationAPIController(IMasterUserLocationBAL MasteruserlocationBal)
        {
            _userLocationBAL = MasteruserlocationBal;
        }


        [HttpPost]
        [Route("Add")]
        public HttpResponseMessage Post(MstLocationUserLocation model)
        {
            ...
            try
            {
                ...
                var modelTxn = _userLocationBAL.Save(model, out ErrorMessage);
        ...
        public HttpResponseMessage Delete(int Id)
        {
            ...
            try
            {
                ...
                var ErrorMessage = string.Empty;
                _userLocationBAL.Delete(Id, out ErrorMessage);
        ...
        public HttpResponseMessage Get(string Id)
        {
            try
            {
                ...
                var obj = _userLocationBAL.Get(int.Parse(Id));
        ...
        public HttpResponseMessage Load(int Id)
        {
            ...
            {
                Log4NetLogger.LogEntry(fileName, nameof(Get), Level.Info.ToString());
                var obj = _userLocationBAL.Load(Id);
                var serialisedData = JsonConvert.SerializeObject(obj);
                if (obj == null)
                    responseObject = BuildResponseObject(HttpStatusCode.NotFound);
                else
        ...
        public HttpResponseMessage GetAll()
        {
            try
            {
                ...
                paginationFilter = GridHelper.GetAllFormatSearchCondition<MstLocationUserLocation>(Request.GetQueryNameValuePairs());
                var commonDAL = new CommonDAL();
                paginationFilter = commonDAL.GetProperPaginationFilter(paginationFilter);

                var result = _userLocationBAL.GetAll(paginationFilter);
                if (result == null)
                    responseObject = BuildResponseObject(HttpStatusCode.NotFound);
                else
                {
