...
namespace UETrack.Application.Web.API.API.BEMS
{
    ...
    {/// <summary>
     ...
     /// </summary>
        private readonly IUserAreaBAL _userAreaBAL;
        private readonly string fileName = nameof(UserAreaAPIController);

        public UserAreaAPIController(IUserAreaBAL userAreaBA)
        {
            _userAreaBAL = userAreaBA;
        }
        ...
        public HttpResponseMessage Post(MstLocationUserAreaViewModel model)
        {
            ...
            try
            {
                ...
                var modelTxn = _userAreaBAL.Save(model, out ErrorMessage);
        ...
        public HttpResponseMessage Delete(int Id)
        {
            try
            {
                ...
                var ErrorMessage = string.Empty;
                _userAreaBAL.Delete(Id, out ErrorMessage);
        ...
        public HttpResponseMessage Get(string Id)
        {
            ...
            {
                Log4NetLogger.LogEntry(fileName, nameof(Get), Level.Info.ToString());
                var obj = _userAreaBAL.Get(int.Parse(Id));
                var serialisedData = JsonConvert.SerializeObject(obj);
                if (obj == null)
                    responseObject = BuildResponseObject(HttpStatusCode.NotFound);
                else
        ...
        public HttpResponseMessage GetAll()
        {
            ...
            try
            {
                ...
                var result = _userAreaBAL.GetAll(paginationFilter);
                if (result == null)
                    responseObject = BuildResponseObject(HttpStatusCode.NotFound);
                else
                {
